...
ReluOperation::ReluOperation(const QnnGpuOpPackage_Node_t* node, Qnn_ErrorHandle_t* status)
  ...
  if (node->configs[0]->numOfInputs != 1u) {
    RETURN(QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT);
  }
  Qnn_Tensor_t input      = node->configs[0]->inputTensors[0];
  size_t numInputElements = 1u;
  for (size_t idx = 0u; idx < input.rank; ++idx) {
    numInputElements *= input.currentDimensions[idx];
  }
  ...
  if (node->configs[0]->numOfOutputs != 1u) {
    RETURN(QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT);
  }
  Qnn_Tensor_t output      = node->configs[0]->outputTensors[0];
  size_t numOutputElements = 1;
  for (size_t idx = 0u; idx < output.rank; ++idx) {
    numOutputElements *= output.currentDimensions[idx];
  }
...
QnnGpu_Kernel_t ReluOperation::setKernelInfo(const QnnGpuOpPackage_Node_t* node,
  ...
  for (size_t idx = 0; idx < input.rank; ++idx) {
    numElements *= input.currentDimensions[idx];
  }

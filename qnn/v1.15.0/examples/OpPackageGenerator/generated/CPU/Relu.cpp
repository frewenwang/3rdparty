...
Qnn_ErrorHandle_t validateOpConfig(Qnn_OpConfig_t opConfig) {
  QNN_CUSTOM_BE_ENSURE_EQ(
      strcmp(opConfig.typeName, "Relu"), 0, QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT);

  QNN_CUSTOM_BE_ENSURE_EQ(opConfig.numOfInputs, 1, QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE);
  QNN_CUSTOM_BE_ENSURE_EQ(opConfig.numOfOutputs, 1, QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE);

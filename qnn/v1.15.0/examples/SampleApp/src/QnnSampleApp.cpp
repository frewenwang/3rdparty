...
sample_app::QnnSampleApp::~QnnSampleApp() {
  ...
  if (m_isBackendInitialized) {
    ...
    if (QNN_BACKEND_NO_ERROR != m_qnnFunctionPointers.qnnInterface.backendTerminate()) {
      QNN_ERROR("Could not terminate backend");
    }
  }
  m_isBackendInitialized = false;
  // Terminate logging in the backend
  if (QNN_SUCCESS != m_qnnFunctionPointers.qnnInterface.logTerminate()) {
...
sample_app::StatusCode sample_app::QnnSampleApp::initialize() {
  ...
  if (log::isLogInitialized()) {
    ...
    QNN_INFO("Initializing logging in the backend. Callback: [%p], Log Level: [%d]",
             logCallback,
             logLevel);
    if (QNN_SUCCESS != m_qnnFunctionPointers.qnnInterface.logInitialize(logCallback, logLevel)) {
  ...
  if (ProfilingLevel::OFF != m_profilingLevel) {
    ...
    if (ProfilingLevel::BASIC == m_profilingLevel) {
      ...
      if (QNN_PROFILE_NO_ERROR != m_qnnFunctionPointers.qnnInterface.profileCreate(
                                      QNN_PROFILE_LEVEL_BASIC, &m_profileBackendHandle)) {
      ...
      if (QNN_PROFILE_NO_ERROR != m_qnnFunctionPointers.qnnInterface.profileCreate(
                                      QNN_PROFILE_LEVEL_DETAILED, &m_profileBackendHandle)) {
...
sample_app::StatusCode sample_app::QnnSampleApp::initializeBackend() {
  auto returnStatus = m_qnnFunctionPointers.qnnInterface.backendInitialize(
      (const QnnBackend_Config_t**)m_backendConfig);
  if (QNN_BACKEND_NO_ERROR != returnStatus &&
      QNN_BACKEND_ERROR_ALREADY_INITIALIZED != returnStatus) {
    QNN_ERROR("Could not initialize backend due to error = %d", returnStatus);
    return StatusCode::FAILURE;
  }
  QNN_INFO("Initialize Backend Returned Status = %d", returnStatus);
...
sample_app::StatusCode sample_app::QnnSampleApp::createFromBinary() {
  ...
  if (caching::StatusCode::SUCCESS !=
      caching::deserializeData(
          m_cachedBinaryPath, graphTensorIdToNamesMap, &m_graphsCount, buffer, bufferSize)) {
    QNN_ERROR("Could not deserialize binary file.");
    returnStatus = StatusCode::FAILURE;
  }
  ...
  if (!populateTensorNamesFromMetadata(graphTensorIdToNamesMap, m_graphsInfo, m_graphsCount)) {
    QNN_ERROR("Failed to populate tensor names from metadata.");
    returnStatus = StatusCode::FAILURE;
  }
  if (StatusCode::SUCCESS == returnStatus &&
...
sample_app::StatusCode sample_app::QnnSampleApp::saveBinary() {
  ...
  if (status == StatusCode::SUCCESS &&
      caching::StatusCode::SUCCESS != caching::serializeData(m_outputPath,
                                                             m_saveBinaryName,
                                                             m_graphsInfo,
                                                             m_graphsCount,
                                                             saveBuffer.get(),

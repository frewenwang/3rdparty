...
iotensor::StatusCode iotensor::IOTensor::copyFromFloatToNative(float* floatBuffer,
  ...
  switch (tensor->dataType) {
    case QNN_DATATYPE_UFIXED_POINT_8:
      datautil::floatToTfN<uint8_t>(static_cast<uint8_t*>(tensor->clientBuf.data),
                                    floatBuffer,
    ...
    case QNN_DATATYPE_UFIXED_POINT_16:
      datautil::floatToTfN<uint16_t>(static_cast<uint16_t*>(tensor->clientBuf.data),
                                     floatBuffer,
                                     tensor->quantizeParams.scaleOffsetEncoding.offset,
                                     tensor->quantizeParams.scaleOffsetEncoding.scale,
                                     datautil::calculateElementCount(dims));
    ...
    case QNN_DATATYPE_UINT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<uint8_t>(static_cast<uint8_t*>(tensor->clientBuf.data),
                                           floatBuffer,
                                           datautil::calculateElementCount(dims))) {
    ...
    case QNN_DATATYPE_UINT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<uint16_t>(static_cast<uint16_t*>(tensor->clientBuf.data),
                                            floatBuffer,
                                            datautil::calculateElementCount(dims))) {
    ...
    case QNN_DATATYPE_UINT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<uint32_t>(static_cast<uint32_t*>(tensor->clientBuf.data),
                                            floatBuffer,
                                            datautil::calculateElementCount(dims))) {
    ...
    case QNN_DATATYPE_INT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<int8_t>(static_cast<int8_t*>(tensor->clientBuf.data),
                                          floatBuffer,
                                          datautil::calculateElementCount(dims))) {
    ...
    case QNN_DATATYPE_INT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<int16_t>(static_cast<int16_t*>(tensor->clientBuf.data),
                                           floatBuffer,
                                           datautil::calculateElementCount(dims))) {
    ...
    case QNN_DATATYPE_INT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castFromFloat<int32_t>(static_cast<int32_t*>(tensor->clientBuf.data),
                                           floatBuffer,
                                           datautil::calculateElementCount(dims))) {
...
iotensor::StatusCode iotensor::IOTensor::populateInputTensor(
  ...
  if (inputDataType == InputDataType::FLOAT && input->dataType != QNN_DATATYPE_FLOAT_32) {
    ...
    datautil::StatusCode status;
    std::tie(status, m_numFilesPopulated, m_batchSize) = datautil::readBatchDataAndUpdateQueue(
        filePaths, dims, input->dataType, static_cast<uint8_t*>(input->clientBuf.data));
    if (datautil::StatusCode::SUCCESS != status) {
      QNN_DEBUG("Failure in datautil::readBatchDataAndUpdateQueue");
      returnStatus = StatusCode::FAILURE;
    }
...
iotensor::StatusCode iotensor::IOTensor::populateInputTensor(
  ...
  if (inputDataType == InputDataType::FLOAT && input->dataType != QNN_DATATYPE_FLOAT_32) {
    ...
    std::tie(returnStatus, length) = datautil::calculateLength(dims, input->dataType);
    if (datautil::StatusCode::SUCCESS != returnStatus) {
      return StatusCode::FAILURE;
    }
    pal::StringOp::memscpy(
        reinterpret_cast<uint8_t*>(input->clientBuf.data), length, buffer, length);
  }
  return StatusCode::SUCCESS;
}
...
iotensor::StatusCode iotensor::IOTensor::setupTensors(
  ...
  *tensors          = (Qnn_Tensor_t*)calloc(1, tensorCount * sizeof(Qnn_Tensor_t));
  ...
  for (size_t tensorIdx = 0; tensorIdx < tensorCount; tensorIdx++) {
    ...
    returnStatus =
        allocateBuffer(reinterpret_cast<uint8_t**>(&((*tensors) + tensorIdx)->clientBuf.data),
                       dims,
                       wrapperTensor.dataType);
    datautil::StatusCode datautilStatus;
    size_t length;
    std::tie(datautilStatus, length) = datautil::calculateLength(dims, wrapperTensor.dataType);
    ...
    if (StatusCode::SUCCESS != returnStatus) {
      ...
      if (nullptr != ((*tensors) + tensorIdx)->clientBuf.data) {
        free(((*tensors) + tensorIdx)->clientBuf.data);
      }
...
iotensor::StatusCode iotensor::IOTensor::convertToFloat(float** out, Qnn_Tensor_t* tensor) {
  ...
  switch (tensor->dataType) {
    case QNN_DATATYPE_UFIXED_POINT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::tfNToFloat<uint8_t>(*out,
    ...
    case QNN_DATATYPE_UFIXED_POINT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::tfNToFloat<uint16_t>(*out,
    ...
    case QNN_DATATYPE_UINT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<uint8_t>(
              *out, reinterpret_cast<uint8_t*>(tensor->clientBuf.data), elementCount)) {
    ...
    case QNN_DATATYPE_UINT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<uint16_t>(
              *out, reinterpret_cast<uint16_t*>(tensor->clientBuf.data), elementCount)) {
    ...
    case QNN_DATATYPE_UINT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<uint32_t>(
              *out, reinterpret_cast<uint32_t*>(tensor->clientBuf.data), elementCount)) {
    ...
    case QNN_DATATYPE_INT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int8_t>(
              *out, reinterpret_cast<int8_t*>(tensor->clientBuf.data), elementCount)) {
    ...
    case QNN_DATATYPE_INT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int16_t>(
              *out, reinterpret_cast<int16_t*>(tensor->clientBuf.data), elementCount)) {
    ...
    case QNN_DATATYPE_INT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int32_t>(
              *out, reinterpret_cast<int32_t*>(tensor->clientBuf.data), elementCount)) {
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensor(Qnn_Tensor_t* output,
  ...
  std::vector<size_t> dims;
  fillDims(dims, output->currentDimensions, output->rank);
  uint8_t* bufferToWrite = reinterpret_cast<uint8_t*>(output->clientBuf.data);
  if (datautil::StatusCode::SUCCESS !=
      datautil::writeBatchDataToFile(
          outputPaths, fileName, dims, output->dataType, bufferToWrite, m_batchSize)) {
    QNN_ERROR("failure in writeBatchDataToFile");
    returnStatus = StatusCode::FAILURE;
  }
  return returnStatus;
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensors(
    ...
    char* graphName,
    Qnn_Tensor_t* outputs,
    qnn_wrapper_api::Qnn_TensorWrapper_t* outputWrappers,
    uint32_t numOutputs,
    iotensor::OutputDataType outputDatatype,
    uint32_t graphsCount,
    std::string outputPath) {
  if (nullptr == outputs || nullptr == outputWrappers) {
  ...
  for (size_t outputIdx = 0; outputIdx < numOutputs; outputIdx++) {
    ...
    if (nullptr != outputWrappers[outputIdx].name && strlen(outputWrappers[outputIdx].name) > 0) {
      outputFilePrefix = std::string(outputWrappers[outputIdx].name);
    } else {
    ...
    if (outputs[outputIdx].dataType == QNN_DATATYPE_FLOAT_32) {
...
iotensor::StatusCode iotensor::IOTensor::allocateAndCopyBuffer(uint8_t** buffer,
  ...
  datautil::StatusCode datautilStatus;
  size_t length;
  std::tie(datautilStatus, length) = datautil::calculateLength(dims, tensor->dataType);
  if (datautilStatus != datautil::StatusCode::SUCCESS) {
    return StatusCode::FAILURE;
  }
  if (StatusCode::SUCCESS != allocateBuffer(buffer, dims, tensor->dataType)) {
    QNN_ERROR("failure in allocateBuffer");
    return StatusCode::FAILURE;
  }
  pal::StringOp::memscpy(
      *buffer, length * sizeof(uint8_t), tensor->clientBuf.data, length * sizeof(uint8_t));
  return StatusCode::SUCCESS;
}

iotensor::StatusCode iotensor::IOTensor::deepCopyQnnTensorInfo(Qnn_Tensor_t* dest,
                                                               Qnn_Tensor_t* src) {
  ...
  dest->rank       = src->rank;
  pal::StringOp::memscpy(&(dest->quantizeParams),
                         sizeof(Qnn_QuantizeParams_t),
                         &(src->quantizeParams),
                         sizeof(Qnn_QuantizeParams_t));
  size_t dimensionsSize   = src->rank * sizeof(uint32_t);
  dest->maxDimensions     = (uint32_t*)malloc(dimensionsSize);
  dest->currentDimensions = (uint32_t*)malloc(dimensionsSize);
  if (nullptr == dest->maxDimensions || nullptr == dest->currentDimensions) {
    if (dest->maxDimensions) {
      free(dest->maxDimensions);
    }
    if (dest->currentDimensions) {
      free(dest->currentDimensions);
    }
    return StatusCode::FAILURE;
  }
  pal::StringOp::memscpy(dest->maxDimensions, dimensionsSize, src->maxDimensions, dimensionsSize);
  pal::StringOp::memscpy(
      dest->currentDimensions, dimensionsSize, src->currentDimensions, dimensionsSize);
  return StatusCode::SUCCESS;
}

...
def generate_compile_packages(backend="CPU"):
    package_gen_tool = os.path.join(QNN_ROOT, 'target', 'x86_64-linux-clang', 'bin', 'qnn-op-package-generator')
...
def convert_model(pb_filename, model_dir, tensorflow_dir, quantize, custom):
    ...
    tools_dir = os.path.join(QNN_ROOT, 'target', 'x86_64-linux-clang', 'bin')
    ...
    if 'QNX_TARGET' not in os.environ:
        raise RuntimeError('QNX_TARGET not set. Please set this env. variable.')

    if 'QNX_HOST' not in os.environ:
        raise RuntimeError('QNX_HOST not set. Please set this env. variable.')

    print('INFO: Compiling model artifacts into shared libraries at: {}'.format(os.path.join(model_dir, 'model_libs')))
    for t in ['aarch64-qnx']:
...
def setup_assets(inception_v3_data_dir, download, convert, quantize, custom, generate):
    ...
    OP_PACKAGE_GEN_DIR = os.path.join(QNN_ROOT, 'examples', 'OpPackageGenerator')

    global OP_PACKAGE_DIR
    OP_PACKAGE_DIR = os.path.join(QNN_ROOT, 'examples', 'OpPackageGenerator', 'generated')

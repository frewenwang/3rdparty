...
static constexpr auto sg_examplePackageName =
    THIS_PKG_NAME_STR;  // package name passed in as compile flag THIS_PKG_NAME
                        // in this example: "examples.OpPackage"
 ...
 * can be set by Qnn_ErrorHandle_t exampleLogSetLevel(QnnLog_Level_t maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t exampleLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
  if (sg_logInitialized) return QNN_LOG_ERROR_ALREADY_INITIALIZED;
...
Qnn_ErrorHandle_t exampleLogTerminate() {
  if (!sg_logInitialized) return QNN_LOG_ERROR_NOT_INITIALIZED;
 ...
 *
 * (version 1.4.0) Qnn_ErrorHandle_t exampleCreateOpImpl (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_OpImpl_t* opImpl) (version 1.4.0)

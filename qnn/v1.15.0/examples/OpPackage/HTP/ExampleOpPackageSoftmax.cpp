...
static constexpr auto sg_opNameSoftmaxD2         = "Softmax.d2";
static constexpr auto sg_opNameSoftmaxD2TableGen = "Softmax.d2.TableGen";
 ...
 * e.g. DEF_PACKAGE_OP((softmaxWithbetaWrapper<Tensor>), sg_opNameSoftmax)
 */
DEF_PACKAGE_OP((softmaxWithbetaWrapper<Tensor>), sg_opNameSoftmax)
DEF_PACKAGE_OP((softmaxWithbetaWrapper<QuantUint8Tensor>), sg_opNameSoftmax)

/*
 * method 2 for defining op with specified cost value (one of "GLACIAL", "SNAIL", "FAST", "FREE")
 * and provided flags syntax: DEF_PACKAGE_OP_AND_COST_AND_FLAGS(F,OP,COST,...) can use
 * zero (default) or more flags, FLAG options are IS_CONST, INHIBIT_CONST_PROP, RESOURCE_HVX,
 ...
 * sg_opNameSoftmax, "SNAIL", Flags::IS_CONST)
 */
DEF_PACKAGE_OP_AND_COST_AND_FLAGS(softmaxD2TablegenImpl, sg_opNameSoftmaxD2TableGen, "FREE", Flags::RESOURCE_HVX)

DEF_PACKAGE_OP_AND_COST_AND_FLAGS((softmaxWithbetaWrapper<PlainFloatTensor>),
                                  sg_opNameSoftmax,
                                  "SNAIL", Flags::RESOURCE_HVX)
 ...
 * e.g. DEF_PACKAGE_OP_AND_COST_AND_FLAGS((softmaxWithbetaWrapper<PlainFloatTensor>),
 * sg_opNameSoftmax, softmaxCostFunc, Flags::IS_CONST)
 */

DEF_PACKAGE_OP_AND_COST_AND_FLAGS((softmaxWithbetaFastWrapper<PlainFloatTensor, QuantUint8Tensor>),
                                  sg_opNameSoftmax,
                                  softmaxCost,
                                  Flags::RESOURCE_HVX)
...
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((softmaxWithbetaFastWrapper<PlainFloatTensor, QuantUint16Tensor>),
                                  sg_opNameSoftmax,
                                  softmaxCost,
                                  Flags::RESOURCE_HVX)
 ...
 * HTP core provides some replacement functions for op package to use, e.g. gen_ConstScalar_f32
 * for more information about optimization rules, please refer to documentation located at
 *   QNN SDK docs/HTP/optimization_grammar.html
 */

/* the following optimization rule utilizes HTP core ops in its replacement ops that are not defined
 * or registered in this package, it is incorporated as part of HTP core as of now
 */
DEF_PACKAGE_OPTIMIZATION(QNN,
                         Op(sg_opNameSoftmax, "In", "Beta", "Axis"),
                         EQ(CONSTVAL_INT("Axis", 0), 3),
                         Op(sg_opNameSoftmax, "In", "Beta"))

// Split on Height. Height 1 special case. Needs to happen at same time for other ops (EARLY-2)
DEF_PACKAGE_OPTIMIZATION(
    EARLY - 2,
    Op(sg_opNameSoftmax, Op(FROM_DEFAULT_PACKAGE("height1_sequence"), "In"), "Beta"),
    OK,
...
DEF_PACKAGE_OPTIMIZATION(
    EARLY - 2,
    Op(sg_opNameSoftmax, Op(FROM_DEFAULT_PACKAGE("height1_sequence"), "In")),
    OK,
    Op(FROM_DEFAULT_PACKAGE("height1_sequence"),
       AUTOSPLIT(1,
                 "I",
...
DEF_PACKAGE_OPTIMIZATION(EARLY,
                         Op(sg_opNameSoftmax, "In"),
                         OK,
                         Op(sg_opNameSoftmax, "In", gen_ConstScalar_f32(1.0f)))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(FROM_DEFAULT_PACKAGE("Reshape"),
       Op(sg_opNameSoftmax, Op(FROM_DEFAULT_PACKAGE("Reshape"), "In"), "Beta")),
    SAME_SHAPE("In", "*"),
    Op(sg_opNameSoftmax, "In", "Beta"))

// this is the case for a reshape followed by a softmax
// since if the softmax is along depth, there is no need to collapse
// the other dimensions into a single one, i.e., (b1,h1,w1,d) -> (1,1,b1xh1xw2,d)
// NOTE: can only do this if the input depth = output depth
DEF_PACKAGE_OPTIMIZATION(
    EARLY + 2,
    Op(sg_opNameSoftmax, LET("ReshapeOut", Op(FROM_DEFAULT_PACKAGE("Reshape"), "In")), "Beta"),
    EQ(DIM_DEPTH("In"), DIM_DEPTH("ReshapeOut")),
    Op(FROM_DEFAULT_PACKAGE("Reshape"), WITH_SIZE("In", Op(sg_opNameSoftmax, "In", "Beta"))))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(FROM_DEFAULT_PACKAGE("Requantize"),
       WITH_OUTPUT_TYPE(DType::QUInt8, 0, DIV(1, 256.0f), Op(sg_opNameSoftmax, "In", "Beta"))))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(sg_opNameSoftmax, "In", "Beta"),
    GT(DIM_BATCHES("*"), 1),
    AUTOSPLIT(0, "I", 1, Op(sg_opNameSoftmax, TYPICAL_SLICE("In", "I"), "Beta")))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    AUTOSPLIT(1, "I", TILE_HEIGHT, Op(sg_opNameSoftmax, TYPICAL_SLICE("In", "I"), "Beta")))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    AUTOSPLIT(2, "I", 256, Op(sg_opNameSoftmax, TYPICAL_SLICE("In", "I"), "Beta")))

DEF_PACKAGE_OPTIMIZATION(
    EARLY + 5,
    Op(sg_opNameSoftmax, "In", "Beta"),
    AND(GT(DIM_WIDTH("*"), 32), GT(DIM_DEPTH("*"), 256)),
    AUTOSPLIT(2, "I", 32, Op(sg_opNameSoftmax, TYPICAL_SLICE("In", "I"), "Beta")))
...
DEF_PACKAGE_OPTIMIZATION(EARLY + 6,
                         ...
                         Op(sg_opNameSoftmaxD2,
                            WITH_SAME_OUTPUT("In",
                                             Op(FROM_DEFAULT_PACKAGE("ForceFormat_Flat"), "In")),
                            "Beta",
                            WITH_TYPE("In",
                                      WITH_SIZE(gen_Shape(1, 1, 1, 256),
                                                Op(sg_opNameSoftmaxD2TableGen,
                                                   gen_ConstScalar_f32(STEPSIZE_OF("In")),
                                                   "Beta")))))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(FROM_DEFAULT_PACKAGE("Quantize"),
       WITH_OUTPUT_TYPE(DType::Float32, 0, 1.0f, Op(sg_opNameSoftmax, "In", "Beta"))))
...
DEF_PACKAGE_OPTIMIZATION(
    LATE + 20,
    Op(sg_opNameSoftmax, Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), "In"), "Beta"),
    OK,
    Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), Op(sg_opNameSoftmax, "In", "Beta")))
...
DEF_PACKAGE_OPTIMIZATION(LATE + 20,
                         Op(FROM_DEFAULT_PACKAGE("flat_to_vtcm"),
                            Op(sg_opNameSoftmaxD2, "In", "Beta", "Table")),
                         OK,
                         Op(sg_opNameSoftmaxD2,
                            WITH_SAME_OUTPUT("In", Op(FROM_DEFAULT_PACKAGE("flat_to_vtcm"), "In")),
                            "Beta",
...
DEF_PACKAGE_OPTIMIZATION(
    LATE + 20,
    Op(sg_opNameSoftmaxD2, Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), "In"), "Beta", "Table"),
    OK,
    Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), Op(sg_opNameSoftmaxD2, "In", "Beta", "Table")))

DEF_PACKAGE_OPTIMIZATION(
    LATE + 10,
    Op(FROM_DEFAULT_PACKAGE("ConvLayer.opt.activations_to_vtcm"),
       Op(sg_opNameSoftmax, Op(FROM_DEFAULT_PACKAGE("ForceFormat_Flat"), "In"), "Beta")),
    OK,
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(FROM_DEFAULT_PACKAGE("SlicePad_shape_inplace.tcm"),
       LET("SOFTMAXQUANTIZE",
           Op(FROM_DEFAULT_PACKAGE("Quantize.tcm"), Op(sg_opNameSoftmax, "In", "Beta"))),
       "Before",
       "Start",
       "Out",
       "Zero"),
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(sg_opNameSoftmaxCrouton, "In", "Beta"))
...
//
// generate a 'delta' or 'rdelta' control vector from a mapping function. The mapping
// function is called with i = 0...127, once for each output lane; it returns the
// index 0...127 of the input lane which is to be routed to that output.
// To indicate a 'don't-care', the function can return -1, or anything in range 128...255.
//
...
void softmaxApproxShortd(float *pout, const uint8_t *pin, float scale, int depth, int nsamples) {
  ...
  for (int n = nsamples; n > 0; n -= block) {
    ...
    pin += block * depth;
    /*
     * Find max. Process up to 4 depth channels per iteration and load into xmax until filled
     * with max values for entire block.
     */
    HVX_Vector xmax;
    HVX_Vector xt = x;
    ...
    for (int k = block; k > 0; k -= 4) {
      ...
      // Only processing up to 4 * depth elements. Permute these so that each depth is in 32 bytes,
      // with unused bytes zeroed out.
      ...
      // Find max for each depth channel. xmax4 should have max val of each depth channel in the
      // first (depth+1)/2 elements of its 32 bytes.
...
void softmaxApproxCrouton(uint8_t **outtab,
  ...
  *optr++             = vExp1;
  ...
  // OK, we have 4 sums, but they're repeated 8 times.  Or at least, they should be...
  // Do reciprocals
  ...
  // Duplicate them
  // We're going to shift right by 16, and want 8 bits left ... so make some adjustments...

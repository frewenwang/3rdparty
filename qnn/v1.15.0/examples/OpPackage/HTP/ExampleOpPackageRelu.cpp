...
static constexpr auto sg_opNameReluMinMax   = "ReluMinMax";
static constexpr auto sg_opNameReluTableGen = "ReluTableGen";
...
DEF_PACKAGE_OP((reluMinMaxImpl<Tensor, Tensor>), sg_opNameReluMinMax)
DEF_PACKAGE_OP(reluTablegenImpl, sg_opNameReluTableGen)
 ...
 */
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((reluImpl<PlainFloatTensor>), sg_opNameRelu, "SNAIL", Flags::RESOURCE_HVX)

/*
 * method 3 for defining op with cost function pointer and provided flags
 * cost function pointer type: typedef float (*cost_function) (const Op * op);
 * syntax: DEF_PACKAGE_OP_AND_COST_AND_FLAGS(F,OP,COST_F,...)
 * e.g. DEF_PACKAGE_OP_AND_COST_AND_FLAGS((reluMinMaxImpl<QUint16CroutonTensor, Tensor >),
 * sg_opNameReluMinMax, "SNAIL", Flags::RESOURCE_HVX)
 */
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(sg_opNameRelu, "X"),
    IS_QUANT_TYPE("X"),
    Op(sg_opNameReluMinMax, "X", gen_ConstScalar_f32(0.0f), gen_ConstScalar_f32(INF)))

DEF_PACKAGE_OPTIMIZATION(
    EARLY,
    Op("Relu6", "X"),
    OK,
    Op(sg_opNameReluMinMax, "X", gen_ConstScalar_f32(0.0f), gen_ConstScalar_f32(6.0f)))

DEF_PACKAGE_OPTIMIZATION(
    EARLY,
    Op("Relu1", "X"),
    OK,
    Op(sg_opNameReluMinMax, "X", gen_ConstScalar_f32(-1.0f), gen_ConstScalar_f32(1.0f)))

DEF_PACKAGE_OPTIMIZATION(EARLY,
                         Op("ReluX", "X", "Max"),
                         OK,
                         Op(sg_opNameReluMinMax,
                            "X",
...
// Drop relu if quant parms if input and output quant params indicate a range of min...max

DEF_PACKAGE_OPTIMIZATION(EARLY,
                         Op(sg_opNameReluMinMax, "In", "Min", "Max"),
                         AND(IS_QUINT8("In"),
                             IS_QUINT8("*"),
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(sg_opNameReluMinMax, "X", "Min", "Max"),
    GT(DIM_BATCHES("*"), 1),
    AUTOSPLIT(0, "I", 1, Op(sg_opNameReluMinMax, TYPICAL_SLICE("X", "I"), "Min", "Max")))
...
DEF_PACKAGE_OPTIMIZATION(EARLY + 3,
                         ...
                         AUTOSPLIT(3,
                                   "I",
                                   CHANNEL_SPLIT_SIZE,
                                   Op(sg_opNameReluMinMax, TYPICAL_SLICE("X", "I"), "Min", "Max")))

DEF_PACKAGE_OPTIMIZATION(
    EARLY + 3,
    Op(sg_opNameRelu, "X"),
    GT(DIM_DEPTH("*"), CHANNEL_SPLIT_SIZE),
    AUTOSPLIT(3, "I", CHANNEL_SPLIT_SIZE, Op(sg_opNameRelu, TYPICAL_SLICE("X", "I"))))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    AUTOSPLIT(1, "I", TILE_HEIGHT, Op(sg_opNameReluMinMax, TYPICAL_SLICE("X", "I"), "Min", "Max")))

DEF_PACKAGE_OPTIMIZATION(EARLY + 4,
                         Op(sg_opNameRelu, "X"),
                         GT(DIM_HEIGHT("*"), TILE_HEIGHT),
                         AUTOSPLIT(1, "I", TILE_HEIGHT, Op(sg_opNameRelu, TYPICAL_SLICE("X", "I"))))
...
DEF_PACKAGE_OPTIMIZATION(
    LATE + 20,
    Op(sg_opNameReluMinMax, Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), "X"), "Min", "Max"),
    NOT(IS_FLOAT16("X")),
    Op(FROM_DEFAULT_PACKAGE("flat_from_vtcm"), Op(sg_opNameReluMinMax, "X", "Min", "Max")))
 ...
 * MANATORY refers to whether this parameter is required to be provided at Qnn_addNode

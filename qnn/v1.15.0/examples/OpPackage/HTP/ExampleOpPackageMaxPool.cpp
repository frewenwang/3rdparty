...
static constexpr auto sg_opNameMaxPool    = "MaxPool_valid";
static constexpr auto sg_opNameMaxPoolTcm = "MaxPool_valid.tcm";
...
// op declarations
DEF_PACKAGE_OP((maxpoolImpl<false, Tensor>), sg_opNameMaxPool)
DEF_PACKAGE_OP((maxpoolOpt<QUint16CroutonTensor>), sg_opNameMaxPool)
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((maxpoolImpl<false, PlainFloatTensor>),
                                  sg_opNameMaxPool,
                                  maxpoolCost,
                                  Flags::RESOURCE_HVX)

DEF_PACKAGE_OP_AND_COST_AND_FLAGS((maxpoolOpt<QUint8CroutonTensor>),
                                  sg_opNameMaxPool,
                                  maxpoolCost,
                                  Flags::RESOURCE_HVX)
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((maxpoolOpt<QUint8CroutonTensor_TCM>),
                                  sg_opNameMaxPoolTcm,
                                  maxpoolCost,
                                  Flags::RESOURCE_HVX)
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((maxpoolOpt<QUint8WideCroutonTensor>),
                                  sg_opNameMaxPool,
                                  maxpoolCost,
                                  Flags::RESOURCE_HVX)
DEF_PACKAGE_OP_AND_COST_AND_FLAGS((maxpoolOpt<QUint8WideCroutonTensor_TCM>),
                                  sg_opNameMaxPoolTcm,
                                  maxpoolCost,
                                  Flags::RESOURCE_HVX)
DEF_PACKAGE_OP((maxpoolOpt<QUint16CroutonTensor_TCM>), sg_opNameMaxPoolTcm)

/* optimization declarations */
DEF_PACKAGE_OPTIMIZATION(
    QNN,
    Op(sg_opNameMaxPool2d, "Act", "filter_size", "stride", "pad_amount"),
...
DEF_PACKAGE_OPTIMIZATION(
    EARLY,
    Op(sg_opNameMaxPool, "Act", "Window", "Stride"),
    OK,
...
DEF_PACKAGE_OPTIMIZATION(EARLY + 1,
                         ...
                         OK,
                         Op(sg_opNameMaxPool, "A", "W", "S"))

DEF_PACKAGE_OPTIMIZATION(
    EARLY + 1,
    Op(sg_opNameMaxPool, "In", "Window", "Stride"),
    ...
    Op(FROM_DEFAULT_PACKAGE("Requantize"),
       WITH_SIZE(gen_ShapeOf("*"),
                 WITH_TYPE("In", Op(sg_opNameMaxPool, "In", "Window", "Stride")))))

DEF_PACKAGE_OPTIMIZATION(
    EARLY + 3,
    Op(sg_opNameMaxPool, "A", "W", "S"),
    GT(DIM_BATCHES("*"), 1),
    AUTOSPLIT(0, "I", 1, Op(sg_opNameMaxPool, TYPICAL_SLICE("A", "I"), "W", "S")))

// Slice 32 or 64 channels at a time, as a workaround
DEF_PACKAGE_OPTIMIZATION(EARLY + 4,
                         Op(sg_opNameMaxPool, "Act", "W", "S"),
                         ...
                         AUTOSPLIT(3,
                                   "I",
                                   CHANNEL_SPLIT_SIZE,
                                   Op(sg_opNameMaxPool, TYPICAL_SLICE("Act", "I"), "W", "S")))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    AUTOSPLIT(
        ...
        Op("MaxPool_valid_tmp",
           AUTOSPLIT_SLICE("Act",
                           AUTOSPLIT_SHAPEFN_APPLY(pool_split_start_valid, "I", "Act", "W", "S"),
                           AUTOSPLIT_SHAPEFN_APPLY(pool_split_size_valid, "I", "Act", "W", "S")),
           "W",
           "S")))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    Op(FROM_DEFAULT_PACKAGE(
           "Decimate"),                  // input: "In" pass through maxpool with stride 1x1 win 2x2
       WITH_SIZE(                        // genereate proper output size
           gen_Shape(DIM_BATCHES("In"),  // batch doesn't change
                     ADD(SUB(DIM_HEIGHT("In"), DIM_HEIGHT("Window")), 1),  //(H-F+2*P )/S)+1
                     ADD(SUB(DIM_WIDTH("In"), DIM_WIDTH("Window")), 1),    //(W-F+2*P )/S)+1
                     DIM_DEPTH("In")),
           Op(sg_opNameMaxPool, "In", "Window", gen_Shape(1, 1, 1, 1))  // change the stride 2x1 to
                                                                        // 1x1
           ),
       "Stride"))
...
DEF_PACKAGE_OPTIMIZATION(LATE + 10,
                         ...
                         Op(FROM_DEFAULT_PACKAGE("ConvLayer.opt.activations_from_vtcm"),
                            Op(sg_opNameMaxPoolTcm, "In", "Window", "Stride")))
...
void maxpoolb2x2wtSliceStride2N00(struct TileData *outdata,
  ...
  for (int h = 0; h < hOut; h += 2) {
    ...
    for (int w = 0; w < wOut; w += 4) {
      ...
      HVX_VectorPair a0 =
          Q6_Wh_vmpa_WubRb(Q6_W_vcombine_VV(inoff, out0), c2e);  // shift to [M00 M01 M02 M03]
      HVX_VectorPair a1 =
          Q6_Wh_vmpa_WubRb(Q6_W_vcombine_VV(inoff, out1), c2e);  // shift to [M10 M11 M12 M13]
...
void maxpoolBSpecialWt(struct TileData *outdata,
                       struct TileData *indata,
                       uint32_t windowSize,
                       uint32_t stride,
                       uint32_t offsets,
  ...
  for (int d = 0; d < dOut; d += 32) {
    if (windowSize == 3 && stride == 2)
      maxpoolb3x3wtSliceStride2N00(outdata, indata, offsets, scalingparams, 2);
    else if (windowSize == 3 && stride == 2 && tileStarts == 0x3f)
      maxpoolb3x3wtSliceStride2N11(outdata, indata, offsets, scalingparams, 2);
    else if (windowSize == 2 && stride == 2)
      maxpoolb2x2wtSliceStride2N00(outdata, indata, offsets, scalingparams, 2);
...
int maxpoolOpt(T &out,
  ...
  if (scaleF < 64.0f &&
      scaleF >= 0.00390625f &&  // scaleF in [2^-8,2^6); otherwise need 32-bit HVX implementation
      strideH == strideW) {
    ...
    } else if constexpr (std::is_base_of<LayoutWideCrouton_8, T>::value) {
      if ((windowH == 3 && windowW == 3 && strideW == 2 && (start == 0 || start == 0x3f)) ||
          (windowH == 2 && windowW == 2 && strideW == 2 && start == 0)) {
        maxpoolBSpecialWt(&tout, &tin, windowW, strideW, offsets, scalingparams, start);
        maxpoolDone = true;
      }
    }

    if constexpr (std::is_base_of<LayoutCrouton_8, T>::value ||
                  std::is_base_of<LayoutWideCrouton_8, T>::value) {

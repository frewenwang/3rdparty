...
typedef enum {
  ...
  /// sets string used for custom oem functionality
 ...
 *
 * @param[out] context A handler to the created context
 *
 * @return Error code:
 *         - QNN_SUCCESS: no error is encountered
 *         - QNN_BACKEND_ERROR_NOT_INITIALIZED: backend not initialized
 *         - QNN_CONTEXT_ERROR_INVALID_ARGUMENT: at least one argument or config option invalid
 ...
 */
QNN_API
Qnn_ErrorHandle_t QnnContext_create(const QnnContext_Config_t** config,
 ...
 *        The binary can be used to re-create context by using QnnContext_createFromBinary().
 *        This function must be called after all entities in the context have been finalized.
 *        Unconsumed tensors are not included in the binary.
 *        Client is responsible for allocating sufficient and valid memory to hold serialized
 *        context content produced by this method.
 *        It is recommended the user calls QnnContext_getBinarySize() to allocate a buffer
 *        of sufficient space to hold the binary.
 *
 * @param[in]  context           A context handle
 *
 ...
 *        The content of a context created in this way cannot be further altered,
 *        meaning *no* new nodes or tensors can be added to the context. Creating context by
 *        de-serializing provided binary is meant for fast content creation, ready to
 *        execute on.
 *
 ...
 *
 * @param[in]  profile          The profile handle on which metrics are
 *                              populated and can be queried.
 *                              Use NULL handle to disable profile collection.
 *                              A handle being re-used would reset and is
 *                              populated with values from the current call.
 *
 * @return Error code:
 *         - QNN_SUCCESS: no error is encountered
 *         - QNN_BACKEND_ERROR_NOT_INITIALIZED: backend not initialized
 *         - QNN_CONTEXT_ERROR_UNSUPPORTED_FEATURE: not supported
 *         - QNN_CONTEXT_ERROR_INVALID_ARGUMENT: _binaryBuffer_ or _context_ is NULL
 *           or _profile_ is not a valid handle
 ...
 *         - QNN_CONTEXT_ERROR_BINARY_CONFIGURATION: binary is not configured for this hardware
 *         - QNN_CONTEXT_ERROR_SET_PROFILE: failed to set profiling info
 *
 * @note DEPRECATED: Use corresponding API through QnnInterface_t.
 */
QNN_API
Qnn_ErrorHandle_t QnnContext_createFromBinary(const void* binaryBuffer,
                                              Qnn_ContextBinarySize_t binaryBufferSize,
                                              Qnn_ContextHandle_t* context,
                                              Qnn_ProfileHandle_t profile);

/**
 * @brief A function to create a context from a stored binary with additional configs. Apart from
 *        configurations, this API is functionally equivalent to QnnContext_createFromBinary().
 *
 * @param[in]  config           Pointer to a NULL terminated array of config option pointers.
 *                              NULL is allowed and indicates no config options are provided.
 *                              All config options have default value, in case not provided.
 ...
 */
QNN_API
Qnn_ErrorHandle_t QnnContext_createFromBinaryWithConfig(const QnnContext_Config_t** config,

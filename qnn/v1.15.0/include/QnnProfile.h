 ...
 *
 * @param[out] profile Qnn Profile object
 *
 * @return Error code
 *         - QNN_SUCCESS: No error encountered
 *         - QNN_COMMON_ERROR_BACKEND_NOT_INITIALIZED: error when QnnProfile_create
 *           called before QnnBackend_initialize. Backend must be initialized before
 ...
 */
QNN_API
Qnn_ErrorHandle_t QnnProfile_create(QnnProfile_Level_t level, Qnn_ProfileHandle_t* profile);

/**
 * @brief Get Qnn profile events collected on the profile handle
 *
 * @param[in]  profile         Qnn_ProfileHandle_t object where profiles were collected
 *
 * @param[out] profileEventIds Returns handles to Qnn profile events
 *                             collected on this profile object
 *
 * @param[out] numEvents       Number of profile events
 *
 * @note profileEvents parameter: profile event memory is associated
 *       with the profile object and released on profile object release
 *       in QnnProfile_free()
 *
 * @return Error code
 *         - QNN_SUCCESS: No error encountered
 ...
 *
 * @note subEventIds parameter: profile event memory is associated
 *       with the profile object and released on profile object release
 *       in QnnProfile_free()
 *
 * @return Error code
 ...
 *
 * @note eventData parameter: eventData memory is associated
 *       with the profile object and released on profile object release
 *       in QnnProfile_free()
 *
 * @return Error code
 ...
 *
 * @note Releasing the profile handle invalidates
 *       the memory returned via calls on this handle such as
 *       QnnProfile_getEvents(), QnnProfile_getSubEvents(),
 *       QnnProfile_getEventData(), etc.
 *
 * @return Error code
 *         - QNN_SUCCESS: No error encountered
 *         - QNN_PROFILE_ERROR_INVALID_HANDLE: _profile_ is not a valid handle.

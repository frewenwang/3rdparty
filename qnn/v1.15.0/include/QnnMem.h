 ...
 *
 * @param[out] memHandles     Array of allocated memory handles, length is _numDescriptors_.
 *                            Same shape as _memDescriptors_ (i.e. memHandles[n] corresponds to
 *                            memDescriptors[n])
 *
 ...
 *         - QNN_COMMON_ERROR_BACKEND_NOT_INITIALIZED: backend not initialized
 ...
 */
QNN_API
Qnn_ErrorHandle_t QnnMem_register(const Qnn_MemDescriptor_t* memDescriptors,
                                  uint32_t numDescriptors,
                                  Qnn_MemHandle_t* memHandles);

/**
 * @brief Deregister a memory which was registered via QnnMem_register and
 *        invalidates memHandle for the given backend.
 *
 ...
 *
 * @return Error code:
 *         - QNN_SUCCESS: memory was successfully deregistered
 *         - QNN_COMMON_ERROR_BACKEND_NOT_INITIALIZED: backend not initialized
 *         - QNN_MEM_ERROR_NOT_SUPPORTED: backend does not support this API

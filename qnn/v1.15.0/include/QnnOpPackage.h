...
// Macros
//=============================================================================
#define QNN_OP_PACKAGE_API_VERSION_MAJOR 1
#define QNN_OP_PACKAGE_API_VERSION_MINOR 4
#define QNN_OP_PACKAGE_API_VERSION_PATCH 0

#define QNN_OP_PACKAGE_RESERVED_INFO_SIZE 13
...
/**
 * @brief Initialize an Op package library's data structures. This function
 *        must be called before any other library functions. Calling multiple
 *        times will result in errors after the first call. This function
 *        can be called again after QnnOpPackage_TerminateFn_t.
 *
 * @param[in] infrastructure Global infrastructure object provided by the backend,
 *                           for use in all operations in the package. This is guaranteed to live at
 *                           least until QnnOpPackage_TerminateFn_t returns, and is safe to cache.
 *
 * @return Error code:
...
/**
 * @brief Terminate an Op package library, freeing all data structures and invalidating
 *        any memory or handles provided by the library. This function may be called
 *        again after a subsequent call to QnnOpPackage_InitFn_t.
 *
 * @return Error code:
 ...
 *
 * @param[out] info Info object for the library. This pointer shall point to memory owned
 *                  by the Op package library and remain valid until QnnOpPackage_TerminateFn_t is
 *                  called on the library. The contents of this struct shall not change before
 ...
 *
 * @param[in]  node                Node object for which kernels should be created. This node may be
 *                                 freed before the created kernels. Neither the node nor it's
 *                                 members should be cached.
 *
 * @param[out] opImpl              Op implementation with executable content to compute the
 *                                 operation specified by _node_. The Op implementation contents
 *                                 will be freed by the backend with QnnOpPackage_FreeOpImplFn_t.
 *
 * @return Error code:
...
/**
 * @brief See QnnLog_initialize() in QnnLog.h for documentation.
 */
...
/**
 * @brief See QnnLog_terminate() in QnnLog.h for documentation.
 */
...
#define QNN_OP_PACKAGE_INTERFACE_INIT                                \
  {                                                                  \
    {                                                                \
        QNN_OP_PACKAGE_API_VERSION_MAJOR, /*interfaceVersion.major*/ \
        QNN_OP_PACKAGE_API_VERSION_MINOR, /*interfaceVersion.minor*/ \
        QNN_OP_PACKAGE_API_VERSION_PATCH  /*interfaceVersion.patch*/ \
    },                                                               \
    {                                                                \
 ...
 *
 * @param[out] interface QNN Op Package interface structure, populated with
 *                       the version and interface methods this Op package provides. Caller
 *                       to manage the lifetime of the pointer, though the contents are to be
 *                       considered invalid if the Op package library is terminated/unloaded.
 *
 * @return Error code:

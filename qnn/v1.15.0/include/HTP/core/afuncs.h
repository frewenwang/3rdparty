...
#define TILEDATA(adrtab, next_tab_col, next_tab_row, h, w, d)                  \
    {                                                                          \
        .addr = (uint8_t **)(adrtab),                                          \
        .offset_t_col = static_cast<uint32_t>(next_tab_col),                   \
        .offset_t_row = static_cast<uint32_t>(next_tab_row),                   \
...
inline __attribute__((unused, always_inline)) NN_INT32_T
saturate_u8(NN_INT32_T val)
{
#ifdef __hexagon__
...
static inline __attribute__((unused, always_inline)) NN_INT32_T
saturate_i16(NN_INT32_T val)
{
#ifdef __hexagon__
 ...
 *
 */
inline __attribute__((unused, always_inline)) constexpr float
flt_getmant(float x)
{
    union {
        float f;
 ...
 */
inline __attribute__((unused, always_inline)) constexpr int32_t
flt_getfrac(float x)
{
    union {
        float f;
    ...
    int32_t m = (uu.u32 & 0x007fffffu) | (1 << 23);
    return m;
}
 ...
 */
inline __attribute__((unused, always_inline)) constexpr float
flt_power2(int iexpo)
{
    int a = (iexpo + 127) & 0xFF;
    union {
        int32_t u32;
        float f;
    } uu = {a << 23};
...
inline constexpr std::pair<int32_t, uint32_t> get_scalefactor(float x)
{
    ...
    uint32_t mask = (1 << MBITS) - 1;
    inval = (inval + (1 << (24 - MBITS - 1))) >>
            (24 - MBITS); // possibly overflows into exponent, but that's OK.
    uint32_t m = ((inval & mask) | (1 << (MBITS - 1)));
 ...
 *
 */
inline __attribute__((unused, always_inline)) constexpr uint32_t
get_scaling_params(float x, int max_sl, int max_sr)
{

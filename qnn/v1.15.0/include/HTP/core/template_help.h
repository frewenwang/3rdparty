...
using UnwrapTypeTemplate =
        typename UnwrapTypeTemplate_struct<Twrap, Tapply>::type;
...
template <typename V, typename ftype, size_t... I>
static inline constexpr auto tuple_from_vec_helper(V &v, ftype f,
                                                   std::index_sequence<I...>)
{
    return std::make_tuple(f(v.at(I))...);
}

template <size_t N, typename ftype, typename V>
static inline constexpr auto tuple_from_vec_N(V &v, ftype f)
{
    return tuple_from_vec_helper(v, f, std::make_index_sequence<N>{});
}

template <typename... TupTypes, typename AggType, size_t... I>
static inline constexpr void tuple_convert_helper(std::tuple<TupTypes...> &tup,
                                                  AggType aggregate,
                                                  std::index_sequence<I...>)
{
    tup = std::make_tuple<TupTypes...>(
            static_cast<std::tuple_element_t<I, std::tuple<TupTypes...>>>(
                    std::get<I>(aggregate))...);
}

template <typename... TupTypes, typename AggType>
static inline constexpr void tuple_convert(std::tuple<TupTypes...> &tup,
                                           AggType aggregate)
{
    // static_assert(sizeof...(TupTypes) == std::tuple_size<AggType>::value);
    return tuple_convert_helper(
            tup, aggregate, std::make_index_sequence<sizeof...(TupTypes)>{});
}

// tuple_map(f, tup): apply f to each element of tup
// Actually, this can take a std::array also.  But it always makes a std::tuple.

template <typename ftype, typename tup, size_t... I>
static inline constexpr auto tuple_map_helper(ftype f, tup &t,
                                              std::index_sequence<I...>)
{
    return std::make_tuple(f(std::get<I>(t))...);
}

template <size_t N, typename ftype, class vec_or_tup>
static inline constexpr auto tuple_map_N(ftype f, vec_or_tup &&t)
{
    return tuple_map_helper(f, t, std::make_index_sequence<N>{});
}

template <typename ftype, class tup>
static inline constexpr auto tuple_map(ftype f, tup &&t)
{
    return tuple_map_N<std::tuple_size<std::decay_t<tup>>::value>(f, t);
}

// tuple_apply(f, tup): call f with arguments contained in tup

template <typename ftype, class tup, size_t... I>
static inline constexpr auto tuple_apply_helper(ftype f, tup &t,
                                                std::index_sequence<I...>)
{
    return f(std::get<I>(t)...);
}

template <size_t N, typename ftype, typename vec_or_tup>
static inline constexpr auto tuple_apply_N(ftype f, vec_or_tup &&t)
{
    return tuple_apply_helper(f, t, std::make_index_sequence<N>{});
}

template <typename ftype, class tup>
static inline constexpr auto tuple_apply(ftype f, tup &&t)
{
    //const size_t tsize = std::tuple_size<tup>::value;
    return tuple_apply_N<std::tuple_size<std::decay_t<tup>>::value>(f, t);
}

// tuple_deref_and_apply: Dereference pointers, turn into references, and apply to f

template <typename ftype, class tup>
static inline constexpr auto tuple_deref_and_apply(ftype f, tup &&t)
{
    return tuple_apply(
            f, tuple_map(
                       [](auto x) {
                           if constexpr (std::is_pointer<decltype(x)>::value) {
                               return std::ref(*x);
                           } else {
                               return x;
                           }
...
// tuple_zip(a,b): take two tuples and change tup<tup<a,b,c>,tup<d,e,f>> into tup<tup<a,d>,tup<b,e>,tup<c,f>>
// Note that this doesn't seem to work well for me when I am trying to initialize a tuple of unique pointers
// as, I guess, there aren't the right constructors for unique pointers to become part of make_tuple...
// Maybe with universal references I could fix this or something?

template <typename tup_a, typename tup_b, size_t... I>
static inline constexpr auto tuple_zip_helper(const tup_a &a, const tup_b &b,
                                              std::index_sequence<I...>)
{
    return std::make_tuple(std::make_tuple(std::get<I>(a), std::get<I>(b))...);
}

template <typename tup_a, typename tup_b>
static inline constexpr auto tuple_zip(const tup_a &a, const tup_b &b)
{
    return tuple_zip_helper(
            a, b,
            std::make_index_sequence<
                    std::tuple_size<std::decay_t<tup_a>>::value>{});
}

#if 0
/* Doesn't seem to be in <tuple> at the moment on my ubuntu install... */
template <typename... Ts>
static inline constexpr auto tuple_apply2(std::tuple<Ts...> &t)
{
    return std::apply(tuple_deref, t);
}
#endif
...
template <template <typename> class Pred, template <typename> class Wrapper,
          typename...>
struct TypeFilter;
...
struct TypeFilter<Pred, Wrapper, Head> {
    using type = std::conditional_t<
            ...
            std::tuple<Wrapper<std::remove_reference_t<
                    Head>>>, // FIXME: remove remove_reference_t
            std::tuple<>>;
};
/*
...
struct TypeFilter<Pred, Wrapper, Head, Tail...> {
    using type = std::conditional_t<
            ...
            typename TupleCons<
                    Wrapper<std::remove_reference_t<Head>>,
...
struct TupFilter<Pred, C<Head, Rest...>> {
    using type = std::conditional_t<
            Pred<Head>::value,
            typename TupleCons<
                    Head, typename TupFilter<Pred, C<Rest...>>::type>::type,
...
template <typename T, typename... Ts> struct Unboxed<const Vector<T, Ts...>> {
    using type = T;
};
template <typename T, typename... Ts> struct Unboxed<Vector<T, Ts...>> {
    using type = T;
};
...
template <class T>
using is_not_const = std::integral_constant<
        bool, !std::is_const<std::remove_pointer_t<unboxed_t<T>>>::value>;
template <class T>
...
template <template <typename...> typename C, typename First, typename... Rest,
          typename Default>
...
constexpr static inline const std::array<T, N>
ptr_to_stdarray_helper(const T *carray, std::index_sequence<I...>)
{
...
template <typename T>
using is_boxed_val =
        std::integral_constant<bool, !std::is_same<T, unboxed_t<T>>::value>;

template <typename T>
using is_unboxed_val =
        std::integral_constant<bool, std::is_same<T, unboxed_t<T>>::value>;

template <typename T>
using is_boxed_const =
        std::integral_constant<bool, is_const<unboxed_t<T>>::value>;

template <typename T>
using is_boxed_nonconst =
        std::integral_constant<bool, is_not_const<unboxed_t<T>>::value>;

template <typename T>
using is_const_graph =
        std::integral_constant<bool, std::is_same<const Graph, T>::value>;

template <typename T>
using is_not_const_graph =
        std::integral_constant<bool, !std::is_same<const Graph, T>::value>;
...
template <typename R, typename... Args> struct ArgsTuples<R(Args...)> {
    static_assert((((std::is_lvalue_reference<Args>::value) ||
    ...
    using const_graph_tup =
            TupFilter_t<is_const_graph, noref_args_tup>; // reference to graph?
    using nonconst_graph_tup =
            TupFilter_t<is_not_const_graph,
                        noref_args_tup>; // not reference to graph
    using fixed_args =
            TupFilter_t<is_unboxed_val, nonconst_graph_tup>; // non-varargs
    using var_args = TupFilter_t<is_boxed_val, noref_args_tup>; // varargs

    using input_tuple =
            TupFilter_t<is_const, fixed_args>; // the inputs as real types
    using output_tuple =
            TupFilter_t<is_not_const, fixed_args>; // the outputs as real types
    using input_ptr_tuple =
            TupMap_t<std::add_pointer_t, input_tuple>; // The inputs as pointers
    using output_ptr_tuple = TupMap_t<std::add_pointer_t,
                                      output_tuple>; // the outputs as pointers
    using output_uniqueptrs_tuple =
            TupMap_t<add_uniqueptr_t,
    ...
    using var_input_tuple =
            TupFilter_t<is_boxed_const, var_args>; // variadic input tuple
    using var_output_tuple =
            TupFilter_t<is_boxed_nonconst, var_args>; // variadic output tuple
    ...
    static constexpr size_t n_inputs =
            std::tuple_size<input_tuple>::value; // number of inputs
    static constexpr size_t n_outputs =
            std::tuple_size<output_tuple>::value; // number of outputs
    static constexpr bool has_graph = (std::tuple_size<const_graph_tup>::value >
                                       0); // does it have a graph operand?
    ...
    static constexpr auto nameArray = GetTypeNames<all_args_tuple>(
            std::make_index_sequence<std::tuple_size_v<all_args_tuple>>{});
 ...
 */
template <typename... T>
using Product_helper = typename Product_helper_struct<T...>::type;
...
struct Product_helper_struct<C<As...>> {
    using type = C<As...>;
};
...
template <template <typename...> typename C, typename... Prefixes,
          typename... Rest>
...
template <template <typename...> typename C, typename... Prefixes,
          typename FirstElem, typename... RestElem, typename... Rest>
struct Product_helper_struct<C<Prefixes...>, C<FirstElem, RestElem...>,
                             Rest...> {
    using type =
            Concat<Product_helper<C<Prefixes...>, C<FirstElem>, Rest...>,
...
#else
template <template <typename...> typename C, typename... Rest>
struct Product_struct<C<Rest...>> {
...
template <bool CHECK_EQ, int LIN_THR, typename EXTRACTOR, typename T,
          typename K>
inline T const *array_search_ordered(T const *arr, int n, K const &k,
                                     EXTRACTOR extr)
{
    T const *p = arr;
...
{
    return array_search_ordered<CHECK_EQ, LIN_THR>(
            arr, n, k, [](T const &x) { return x; });
}
...
{
    return const_cast<T *>(
            array_search_ordered<CHECK_EQ, LIN_THR, EXTRACTOR, T>(
...
{
    return const_cast<T *>(array_search_ordered<CHECK_EQ, LIN_THR>(
            const_cast<T const *>(arr), n, k, [](T const &x) { return x; }));
}
...
T2 const *sorted_pair_lookup(std::vector<std::pair<T1, T2>> const &v,
                             T1 const &k)
{
    int n = v.size();
    auto const *arrp = v.data();
    auto const *posn = array_search_ordered<false, 6>(
            arrp, n, k, [](decltype(*arrp) const &p) { return p.first; });
    if (posn >= &arrp[n] || posn->first != k)
        return nullptr;
    return &posn->second;
}
...
std::tuple<T1, Tx...> const *
sorted_tuple_lookup(std::vector<std::tuple<T1, Tx...>> const &v, T1 const &k)
{
    int n = v.size();
    auto const *arrp = v.data();
    auto const *posn = array_search_ordered<false, 4>(
            arrp, n, k,
            [](decltype(*arrp) const &tup) { return std::get<0>(tup); });
    if (posn >= &arrp[n] || std::get<0>(*posn) != k)
        return nullptr;
    return posn;
}

...
class string_tag_t {
  ...
  public:
    ...
    bool operator==(std::string_view x)
            const; //{ return registry_t::unmap_sv(skey)==x;}
    bool operator==(std::string const &x)
            const; // { return registry_t::unmap_sv(skey)==x;}
    bool operator==(char const *x)
            const; //{ return registry_t::unmap_sv(skey)==std::string_view(x);}
    template <class T> bool operator!=(T &&other) const
    {
    ...
    static inline string_tag_t nullobj()
    {
        return string_tag_t((string_key) nullptr);
    }
...
//
__attribute__((visibility("default"))) opname_tag_t
make_opname(char const *opname, char const *prefix = THIS_PKG_NAME_STR);
...
template <typename T>
bool get_option_value(Options const &ops, hnnx::opname_tag_parm_t optname,
                      T &result);
...
template <> struct hashmap_traits<hnnx::string_tag_t, false> {
    static constexpr bool valid = true;
    static inline hnnx::string_tag_t generate_null()
    {
        return hnnx::string_tag_t::nullobj();
    }
    static inline bool is_null(hnnx::string_tag_t k)
    {
        return k == hnnx::string_tag_t::nullobj();
    }
};

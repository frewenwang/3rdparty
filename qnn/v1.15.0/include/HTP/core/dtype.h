...
dtype_info __attribute__((visibility("default")))
DType_info(DType d); // in graph.cc
...
template <DType DT> dtype_info constexpr inline dtype_info_for()
{
    ...
    return dtype_info{
            .elbytes = sizeof(typename traits::element_type),
            .is_quant = traits::is_quant,
            .is_float = traits::is_float,
            .is_signed =
                    (std::is_signed<typename traits::element_type>::value ? 1
                                                                          : 0)};
}
template <> dtype_info constexpr inline dtype_info_for<DType::UNKNOWN>()
{
    return dtype_info{
            .elbytes = 0, .is_quant = 0, .is_float = 0, .is_signed = 0};
}

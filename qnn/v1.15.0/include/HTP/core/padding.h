...
template <Idx Rank> class NoPadding {
  public:
    ...
    inline constexpr std::array<Idx, Rank>
    pad_coords(const std::array<Idx, Rank> &coords,
    ...
    template <typename PadT>
    inline constexpr std::array<Idx, Rank>
    pad_coords(const std::array<SIdx, Rank> &coords,
               const std::array<PadT, Rank> &left_padding) const
    {
        std::array<Idx, Rank> ret;
        std::transform(coords.cbegin(), coords.cend(), ret.begin(),
                       [](SIdx x) -> Idx { return x; });
        return ret;
    }
};
...
template <Idx Rank> class Padding {
  public:
    ...
    template <typename PadT>
    inline const std::array<Idx, Rank>
    pad_coords(const std::array<Idx, Rank> &coords,
               const std::array<PadT, Rank> &left_padding) const
    {
        std::array<Idx, Rank> ret;
        std::transform(left_padding.begin(), left_padding.end(), coords.begin(),
                       ret.begin(), std::plus<Idx>());
        return ret;
    }
    template <typename PadT>
    inline const std::array<Idx, Rank>

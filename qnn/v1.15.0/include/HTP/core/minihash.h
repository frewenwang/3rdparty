...
inline int ceiling_log2(size_t value)
{
    ...
    if constexpr (sizeof(size_t) <= sizeof(unsigned long)) {
        int clz = __builtin_clzl((unsigned long)value - 1);
        return 8 * sizeof(unsigned long) - clz;
    } else {
        int clz = __builtin_clzll((unsigned long long)value - 1);
...
template <> struct findhash<unsigned> {
    ...
    {
        uint64_t bigprod = uint64_t(n) * 0x740F1DE9;
...
template <> struct findhash<unsigned long long> {
    ...
    {
        unsigned upperhash = mulu32_modular(0x192E2101u, (unsigned)(n >> 32));
...
template <typename T> struct findhash<T *> {
    inline uint32_t operator()(T *ptr) const
    {
        return findhash<size_t>()((size_t)ptr);
    }
};
...
struct simple_raw_entry : public simple_key<Key> {
    ...
    static inline bool is_null(Key k)
    {
        return simple_key<Key>::traits::is_null(k);
    }
    ...
    inline T const &value_field() const
    {
        return *reinterpret_cast<T const *>(value_slot);
    }
...
class hashmap {
    ...
    typedef typename std::conditional<ERASE_OK, erasable_raw_entry<Key, T>,
                                      simple_raw_entry<Key, T>>::type
            raw_entry_t;

    static constexpr bool T_needs_dtor =
  ...
  public:
    ...
    typedef typename std::conditional<is_set, Key,
                                      std::pair<const Key, T>>::type value_type;
  ...
  private:
    ...
    class hmap_kiterator {
      ...
      protected:
        ...
        hmap_kiterator(hashmap const *obj, table_iter_type posn)
            : m_object(const_cast<hashmap *>(obj)),
              m_posn(posn - obj->m_table.begin())
        {
        }
      ...
      public:
        ...
        bool operator==(hmap_kiterator const &other) const
        {
    ...
    class hmap_iterator : public hmap_kiterator {
      ...
      protected:
        ...
        hmap_iterator(hashmap *obj, table_iter_type posn)
            : hmap_kiterator(obj, posn)
        {
        }
        ...
        hmap_iterator
        coerce_from(typename otherhash::hmap_kiterator const &other)
        {
      ...
      public:
        ...
        { // pre-inc
            return static_cast<hmap_iterator &>(
                    this->hmap_kiterator::operator++());
        }
  ...
  protected:
    template <typename otherhash>
    static hmap_iterator
    coerce_iter(typename otherhash::hmap_kiterator const &other)
    {
        return hmap_iterator((hashmap *)other.m_object, other.m_posn);
    }
  ...
  public:
    ...
    explicit hashmap(size_t n_entries)
        : m_hashN(0), m_log2N(-1), m_entries(0), m_deleted(0)
    {
    ...
    {
        return hmap_kiterator{const_cast<hashmap *>(this),
                              find_next_for_iter(m_hashN)};
    }
    ...
    {
        return hmap_kiterator{const_cast<hashmap *>(this), -1};
    }
    ...
    {
        table_kiter_type iter = find_key(k);
        return hmap_iterator{this, (iter == m_table.end())
                                           ? -1
                                           : int(iter - m_table.begin())};
    }
    const_iterator find(Key const &k) const
    {
        table_kiter_type iter = find_key(k);
        return hmap_kiterator{this, (iter == m_table.end())
                                            ? -1
                                            : int(iter - m_table.begin())};
    }

    // this may invalidate iterators by enlarging the table,
    // even if the value is already there; but the one returned is always valid.
    template <class... Args>
    std::pair<iterator, bool> emplace(Key const &k, Args &&...args)
    {
        table_iter_type titer = find_key_for_ins(k);
        std::pair<iterator, bool> result{iterator{this, titer}, false};

        if (!titer->is_inuse()) {
            [[maybe_unused]] bool was_deleted = titer->is_deleted();
            result.second = true;
    ...
    T &at(Key const &k)
    {
        table_kiter_type titer = find_key(k);
        if (titer == m_table.end())
            throw std::out_of_range("minimap::at");
        return const_cast<T &>(titer->value_field());
    }
    T const &at(Key const &k) const
    {
        return const_cast<hashmap &>(*this).at(k);
    }
    ...
    T &operator[](Key const &k)
    {
        ...
        table_iter_type titer = find_key_for_ins(k);
        if (!titer->is_inuse()) {
            bool was_deleted = titer->is_deleted();
            titer->emplace_new(k);
            if constexpr (ERASE_OK) {
                if (was_deleted)
                    m_deleted--;
    ...
    iterator erase(iterator const &posn)
    {
        ...
        if constexpr (ERASE_OK) {
            int p = posn.m_posn;
            table_kiter_type entp = m_table.cbegin() + p;
            assert(posn.m_object == this && (size_t)p < m_hashN &&
                   entp->is_inuse());
            erase_inner(entp);
            return hmap_iterator{this, find_next_for_iter(m_hashN)};
        } else {
  ...
  protected:
    ...
    // find_key_for_ins is lookup_key_template with insert;
    // checks first if there is room for insert.
    table_iter_type find_key_for_ins(Key const &k)
    {
        if (m_entries * 2 >= m_hashN) { // including 'no table' state
            rehash_table(true);
        }
        return lookup_key_template<true>(k);
    }
    ...
    {
        // how big to make it?
        unsigned sz = size(); // number of non-deleted entries
        if (!ERASE_OK)
            growing = true; // don't need 'shrink' code path
        int new_log2 = growing ? (ceiling_log2(sz | 15) + 2)
                               : (ceiling_log2(sz + 3 + (sz >> 1)) + 1);
        std::vector<raw_entry_t> old_table;
        old_table.swap(m_table);
        make_new_table(new_log2);
        size_t new_count = 0;
        size_t old_size = old_table.size();
        raw_entry_t *old_table_p = old_table.data();
        for (size_t i = 0; i < old_size; i++) {
            raw_entry_t &old_entry = old_table_p[i];
            if (old_entry.is_inuse()) {
                new_count++;
                Key k = old_entry.first;
                table_iter_type new_loc = lookup_key_template<true>(k);
    ...
    {
        raw_entry_t const *table_p = m_table.data();
        assert(posn <= int(m_hashN));
        while (--posn >= 0) {
            if (table_p[posn].is_inuse())
                return posn;
        }
        return -1;
    }
    ...
    void copy_from(hashmap const &other)
    {
        ...
        //
        raw_entry_t const *other_p = other.m_table.data();
        raw_entry_t *this_p = m_table.data();
...
class hashset : private hashmap<typename value_proxy<Key, true>::type, no_value,
                                ERASE_OK, HSH> {
  ...
  public:
    ...
    inline size_t count_deleted() const noexcept
    {
        return hashimpl::count_deleted();
    }
    ...
    bool contains(Key const &k) const
    {
        return hashimpl::contains(key_down(k));
    }
    iterator find(Key const &k) { return iter_up(hashimpl::find(key_down(k))); }
    const_iterator find(Key const &k) const
    {
        return iter_up(hashimpl::find(key_down(k)));
    }
    ...
    iterator erase(iterator const &posn)
    {
        return iter_up(hashimpl::erase(iter_down(posn)));
    }
};

...
enum AllocOptions {
    uncached = 1 // override MemoryClass to UnCached.
};
...
class Allocator {
  public:
    ...
    void *tracked_aligned_alloc(size_t al, size_t bytes);
    void tracked_free(void *aligned_ptr) noexcept;

    virtual void allocate_n(void **arrp, size_t n, size_t block_size,
                            size_t alignment, MemoryClass memclass,
                            unsigned options) = 0;
    ...
    virtual void allocate_persistent_blocks(void **table, size_t nblocks,
                                            size_t block_size, size_t alignment,
                                            unsigned options) = 0;

    inline void *allocate(const void *oldval, size_t block_size,
                          size_t alignment, MemoryClass memclass,
                          unsigned options)
    {
        void *tmp = (void *)oldval;
        allocate_n(&tmp, 1, block_size, alignment, memclass, options);
        return tmp;
    }
	 ...
	 */
    virtual void serialize_block(Serializer &, const void *addr,
    ...
    virtual void serialize_blocks(Serializer &, const void *const *table,
                                  const size_t block_size, size_t nblocks);
    virtual void deserialize_blocks(Deserializer &, const void **table,
                                    size_t nblocks);

    static inline constexpr size_t fixup_alignment(size_t align)
    {
        static_assert(MIN_ALIGN >= 8 && (MIN_ALIGN & (MIN_ALIGN - 1)) == 0,
                      "bad MIN_ALIGN");
        static_assert(MAX_ALIGN >= MIN_ALIGN &&
                              (MAX_ALIGN & (MAX_ALIGN - 1)) == 0,
                      "bad MAX_ALIGN");
        if (MIN_ALIGN < MAX_ALIGN) {
            return std::max<size_t>(MIN_ALIGN,
                                    std::min<size_t>(MAX_ALIGN, align));
        } else {
            return MIN_ALIGN;
        }
    }

    static inline constexpr size_t round_up_align(size_t n, size_t align)
    {
...
class FakeAllocator : public Allocator {
  public:
    FakeAllocator(Allocator::Mode mode_in, Graph &graph_in)
...
class MemPoolRunTimeAccessor {
  ...
  public:
    MemPoolRunTimeAccessor(void *spill_area_in, fa::PoolDesc const *pt,
                           unsigned pt_size)
        : spill_area(spill_area_in), pool_table(pt), max_pool_id(pt_size)
    {
    }
    MemPoolRunTimeAccessor()
        : spill_area(nullptr), pool_table(nullptr), max_pool_id(0)
    {
    }

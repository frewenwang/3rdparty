...
API_C_FUNC std::string
        API_FUNC_NAME(combine_pkg_op_name)(const char *package_name,
                                           const char *op_name);
 ...
 */
std::vector<ParamConfig_t> *add_to_param_map(ParamMap_t &pmap,
                                             std::string_view package_op_name);

// adds a new op parameter entry into a vector of parameter configs
void add_package_individual_param_config(std::vector<ParamConfig_t> *pvec,
                                         const char *param_name, bool mandatory,
                                         void *default_val);

// base conditon for hnnx::add_package_param_configs_base
void add_package_param_configs_base(std::vector<ParamConfig_t> *pvec);
...
{
    add_package_individual_param_config(pvec, param_name, mandatory,
                                        default_val);
...
template <typename... T>
void add_package_param_configs(ParamMap_t &pmap,
                               std::string_view package_op_name,
                               const char *param_name, bool mandatory,
                               void *default_val, T &&...args)
{
...
#define INIT_PACKAGE_PARAM_ORDER_DEF()                                         \
    __attribute__((visibility("hidden"))) hnnx::ParamMap_t &                   \
    ...
    extern "C" {                                                               \
    void clear_package_param_order_storage_map_func()                          \
    {                                                                          \
...
#define DEF_PACKAGE_PARAM_ORDER(OP, PARAM1, MANDATORY1, DEFAULT1, ...)         \
    [[maybe_unused]] static bool CTRICKS_PASTER(_PKG_PARAM_ORDER_REG_,         \
                                                __LINE__) =                    \
            (hnnx::add_package_param_configs(                                  \
                     current_package_param_order_storage_map_func(),           \
...
#define REGISTER_PACKAGE_PARAM_ORDERS()                                        \
    if (getPkgParamTmpMap().find(std::string(THIS_PKG_NAME_STR)) ==            \
        getPkgParamTmpMap().end())                                             \
        getPkgParamTmpMap()[std::string(THIS_PKG_NAME_STR)] =                  \
                &current_package_param_order_storage_map_func();

#define LIST_PACKAGE_AXIS_PARAMS(...)                                          \
    __attribute__((visibility("hidden"))) std::set<std::string>                \
            &current_package_param_axes_set_func()                             \
    {                                                                          \
        static std::set<std::string> axes;                                     \
        return axes;                                                           \
    }                                                                          \
    extern "C" {                                                               \
    void clear_package_param_axes_set_func()                                   \
    {                                                                          \
        current_package_param_axes_set_func().clear();                         \
    }                                                                          \
    }                                                                          \
    [[maybe_unused]] static bool CTRICKS_PASTER(_PKG_AXIS_PARAMS_, __LINE__) = \
            (hnnx::add_package_axis_params(                                    \
                     current_package_param_axes_set_func(), ##__VA_ARGS__),    \
             true);                                                            \
    std::unordered_map<std::string, std::set<std::string> *>                   \
            &getPkgParamAxesTmpMap();

#define REGISTER_PACKAGE_AXIS_PARAMS()                                         \
    if (getPkgParamAxesTmpMap().find(std::string(THIS_PKG_NAME_STR)) ==        \
        getPkgParamAxesTmpMap().end())                                         \
        getPkgParamAxesTmpMap()[std::string(THIS_PKG_NAME_STR)] =              \
                &current_package_param_axes_set_func();

#define LIST_PACKAGE_PER_CHANNEL_QUANTIZED_OPS(...)                            \
    __attribute__((visibility("hidden"))) std::set<std::string>                \
            &current_package_per_channel_quantized_ops_set_func()              \
    {                                                                          \
        static std::set<std::string> per_channel_ops;                          \
        return per_channel_ops;                                                \
    }                                                                          \
    extern "C" {                                                               \
    void clear_package_per_channel_quantized_ops_set_func()                    \
    {                                                                          \
        current_package_per_channel_quantized_ops_set_func().clear();          \
    }                                                                          \
    }                                                                          \
    [[maybe_unused]] static bool CTRICKS_PASTER(_PKG_PER_CHANNEL_OPS_,         \
                                                __LINE__) =                    \
            (hnnx::add_package_per_channel_ops(                                \
                     current_package_per_channel_quantized_ops_set_func(),     \
                     ##__VA_ARGS__),                                           \
             true);                                                            \
    std::unordered_map<std::string, std::set<std::string> *>                   \
            &getPkgPerChannelOpsTmpMap();

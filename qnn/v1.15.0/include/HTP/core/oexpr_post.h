...
class opdef_accessor {
  public:
    ...
    static OutputDef const &get_outputdef(ECtx &e,
                                          hnnx::operand_tag_parm_t optag);
    static OutputDef const &get_outputdef(ECtx &e, OpRef);
    static Split_Context const &
    lookup_split(ECtx &e, hnnx::split_context_tag_t const &tag);

    static OpRef get_input_of(ECtx &e, OpDef const *, int idx);
    static OpRef get_output_of(ECtx &e, OpRef, int idx);
    template <typename T>
    static T get_option(ECtx &e, hnnx::opname_tag_parm_t name);
    static void show_debug_message(ECtx &e, char const *why, char const *str)
    {
#ifdef WITH_OPT_DEBUG
        const_cast<constraint_lib::Constraint &>(e).show_debug_message(why,
                                                                       str);
#endif
    }
};
...
template <typename OEXPR_TYPE>
inline OpDef const &opdef_accessor::get_opdef_oexpr(ECtx &e,
                                                    OEXPR_TYPE const &oexp)
{
...
template <typename T> struct opwrapper_helper {
    static_assert(false && sizeof(T),
                  "wrap_opexpr instantiated on unsupported type");
};
...
template <> struct opwrapper_helper<hnnx::operand_tag_t> {
    static auto wrap(hnnx::operand_tag_parm_t p)
    {
        return opexpr<OpVnt::parm, void>(p);
    }
};
// wrapping an opexpr
...
{
    return opwrapper_helper<std::remove_reference_t<T>>::wrap(forward<T>(p));
}
...
template <typename TOP, typename TEXPR> auto INPUT_OF(TOP &&opa, TEXPR &&exprb)
{
    ...
    return opexpr<OpVnt::input_of, std::tuple<decltype(wa), decltype(wb)>>(wa,
                                                                           wb);
}
/////////////////////////////////////////
...
template <typename TOP, typename TEXPR> auto OUTPUT_OF(TOP &&opa, TEXPR &&exprb)
{
    auto wa = wrap_opexpr(std::forward<TOP>(opa));
    auto wb = wrap_param_to<int>(std::forward<TEXPR>(exprb));
    return opexpr<OpVnt::output_of, std::tuple<decltype(wa), decltype(wb)>>(wa,
                                                                            wb);
}
...
template <typename T> class expr<Variant::config, T> : public opdef_accessor {
  ...
  public:
    ...
    otype eval(ECtx &ectx) const
    {
        return opdef_accessor::get_option<T>(ectx, m_option_name);
    }
};
...
template <> class expr<Variant::config, bool> : public opdef_accessor {
  ...
  public:
    ...
    otype eval(ECtx &ectx) const
    {
        return opdef_accessor::get_option<int>(ectx, m_option_name) != 0;
    }
};
...
struct property_extract_rank : private opdef_accessor {
    ...
    {
        return get_outputdef(e, oref).rank;
    }
};
struct property_extract_dtype : private opdef_accessor {
    ...
    inline otype eval(ECtx &e, OpRef oref) const
    {
        return get_outputdef(e, oref).dtype;
    }
};
...
struct property_extract_stepsize : private opdef_accessor {
    ...
    inline otype eval(ECtx &e, OpRef oref) const
    {
        return get_outputdef(e, oref).stepsize;
    }
};
...
template <typename ITYPE> struct dim_extractor : public property_extract_dim {
    ...
    constexpr dim_extractor(ITYPE const &iexpr)
        : m_iexpr(wrap_param_to<int>(iexpr))
    {
    }
};
...
class expr<Variant::property, std::tuple<OPEX, PROPEXT>> {
  ...
  public:
    ...
    constexpr expr(OPEX const &opexp, T &&xparm)
        : m_opexpr(opexp), m_propex(forward<T>(xparm))
    {
    }
...
template <typename TOP> inline constexpr auto RANK_OF(TOP &&op)
{
    ...
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_rank>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto DTYPE_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_dtype>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto ELEMENTSIZE_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_elementsize>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto ZERO_OFFSET_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_zero_offset>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto STEPSIZE_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_stepsize>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto INPUTS_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_n_inputs>>(
            op_wrapped);
}
...
template <typename TOP> inline constexpr auto OUTPUTS_OF(TOP &&op)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped), property_extract_n_outputs>>(
            op_wrapped);
}
...
inline constexpr auto DIM_OF(TOP &&op, T &&index)
{
    auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
    return expr<Variant::property,
                std::tuple<decltype(op_wrapped),
                           dim_extractor<std::remove_reference_t<T>>>>(
            op_wrapped, forward<T>(index));
}
...
class expr<Variant::opcompare, std::tuple<OPEXA, OPEXB, OPCMP>>
    : public property_extract_dim {
  ...
  public:
    ...
    expr(OPEXA const &opexpa, OPEXB const &opexpb)
        : m_opexpra(opexpa), m_opexprb(opexpb)
    {
    }
...
inline auto SAME_OP(TOPA &&opa, TOPB &&opb)
{
    ...
    return expr<Variant::opcompare,
                std::tuple<decltype(opa_wrapped), decltype(opb_wrapped),
                           op_compare_same_op>>(opa_wrapped, opb_wrapped);
}
...
inline auto SAME_ENCODING(TOPA &&opa, TOPB &&opb)
{
    ...
    return expr<Variant::opcompare,
                std::tuple<decltype(opa_wrapped), decltype(opb_wrapped),
                           op_compare_same_encoding>>(opa_wrapped, opb_wrapped);
}
...
class expr<Variant::getconst, tuple<GETCONST, OPEXPR, IEXPR>>
    : private opdef_accessor {
    OPEXPR m_opexpr;
    IEXPR m_iexpr;
  ...
  public:
    ...
    constexpr expr(OPEXPR const &oexp, IEXPR const &iexpr)
        : m_opexpr(oexp), m_iexpr(iexpr)
    {
    }
};

template <typename GETCONST, typename OPEXPR, typename IEXPR, typename IEXPR2>
class expr<Variant::getconst, tuple<GETCONST, OPEXPR, IEXPR, IEXPR2>>
    : private opdef_accessor {
    OPEXPR m_opexpr;
    IEXPR m_iexpr;
    IEXPR2 m_iexpr2;
...
inline constexpr auto make_getconst_expr(OPEXPR &&opexp, IEXPR &&iexpr)
{
    return expr<Variant::getconst,
                tuple<GETCONST, std::remove_reference_t<OPEXPR>,
                      std::remove_reference_t<IEXPR>>>(
...
constexpr auto CONSTVAL_INT(OPEXPR &&opexp, IEXPR &&idx)
{
    ...
    auto wi = wrap_param(forward<IEXPR>(idx));
    return make_getconst_expr<getconst_int>(std::move(op_wrapped),
                                            std::move(wi));
}
...
constexpr auto CONSTVAL_INT(OPEXPR &&opexp, IEXPR &&idx, IEXPR2 &&idx2)
{
    auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
    auto wi = wrap_param(forward<IEXPR>(idx));
    auto wi2 = wrap_param(forward<IEXPR2>(idx2));
    return make_getconst_expr<getconst_int2>(std::move(op_wrapped),
                                             std::move(wi), std::move(wi2));
}
...
constexpr auto CONSTVAL_INT_VALID(OPEXPR &&opexp, IEXPR &&idx)
{
    auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
    auto wi = wrap_param(forward<IEXPR>(idx));
    return make_getconst_expr<getconst_int_valid>(std::move(op_wrapped),
                                                  std::move(wi));
}
...
constexpr auto CONSTVAL_INT_VALID(OPEXPR &&opexp, IEXPR &&idx, IEXPR2 &&idx2)
{
    ...
    auto wi = wrap_param(forward<IEXPR>(idx));
    auto wi2 = wrap_param(forward<IEXPR2>(idx2));
    return make_getconst_expr<getconst_int2_valid>(
            std::move(op_wrapped), std::move(wi), std::move(wi2));
}
...
constexpr auto CONSTVAL_FLOAT(OPEXPR &&opexp, IEXPR &&idx)
{
    auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
    auto wi = wrap_param(forward<IEXPR>(idx));
    return make_getconst_expr<getconst_float>(std::move(op_wrapped),
                                              std::move(wi));
}
...
constexpr auto CONSTVAL_FLOAT_VALID(OPEXPR &&opexp, IEXPR &&idx)
{
    auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
    auto wi = wrap_param(forward<IEXPR>(idx));
    return make_getconst_expr<getconst_float_valid>(std::move(op_wrapped),
                                                    std::move(wi));
}
...
inline constexpr auto SELECT(SEL &&s, A &&a, B &&b)
{
    ...
    if constexpr (Replacement::is_Op_type<A>() ||
                  Replacement::is_Op_type<B>()) {
        static_assert(Replacement::is_Op_type<A>() &&
                              Replacement::is_Op_type<B>(),
                      "bad SELECT parameters");
        auto ws = wrap_param_to<bool>(forward<SEL>(s));
        auto wa = wrap_opexpr(forward<A>(a));
        auto wb = wrap_opexpr(forward<B>(b));
        return make_opselect(ws, wa, wb);
    } else {
        auto ws = wrap_param_to<bool>(forward<SEL>(s));
        auto wa = wrap_param(forward<A>(a));
        auto wb = wrap_param(forward<B>(b));
...
struct expr<Variant::message, tuple<MODE, CONDEXPR>> : private opdef_accessor {
    ...
    typedef bool otype;
    expr(char const *m, CONDEXPR &&cond)
        : m_condition(std::move(cond)), m_message(m)
    {
    }
    ...
    inline bool eval(ECtx &e) const
    {
        if constexpr (std::is_same_v<MODE, msg_message>) {
            ...
            if constexpr (std::is_same_v<MODE, msg_message_if>) {
                if (result)
                    show_debug_message(e, "MESSAGE_IF", m_message.c_str());
            } else {
                if (!result)
                    show_debug_message(e, "MESSAGE_IFNOT", m_message.c_str());
            }
            return result;
        }
    }
...
{
    return expr<Variant::message, tuple<MODE, std::remove_reference_t<CND>>>(
            str, forward<CND>(cond));
}
#endif
...
struct expr<Variant::external, tuple<FUNC, INPARMS>> : private opdef_accessor {
    ...
    using otype = decltype(apply_cst_function<FUNC, INPARMS>(
            fake_ectx(), m_function, OpRef(), m_other_operands,
    ...
    inline otype eval(ECtx &e) const
    {
        ...
        return apply_cst_function<FUNC, INPARMS>(
                e, m_function, target, m_other_operands,
                std::make_index_sequence<NPARMS>());
    }
};
...
{
    auto parmpack = std::make_tuple(wrap_param(forward<Args>(args))...);
    return expr<Variant::external, tuple<FUNC, decltype(parmpack)>>(
            f, optag, std::move(parmpack));
}
...
using oExp::IS_POW2;
using oExp::MESSAGE, oExp::MESSAGE_IF, oExp::MESSAGE_IFNOT;
...
using oExp::ROUNDUP;
using oExp::SAME_OP, oExp::SAME_ENCODING;
using oExp::SELECT;
...
using oExp::OPTION_INT, oExp::OPTION_UINT, oExp::OPTION_FLOAT,
        oExp::OPTION_BOOL;

...
class MemBlockEnumerator {
  public:
    ...
    virtual void supply_blocks_func(Tensor const *tensp, int memclass,
                                    void *const *ptr, size_t num) = 0;
    ...
    inline void supply_blocks(Tensor const *tensp, MemoryClass mc,
                              void *const *ptr, size_t num)
    {
...
class MemBlockEnumToSet : public MemBlockEnumerator {
  ...
  public:
    explicit MemBlockEnumToSet(blockid_set_t &s, int mclass_sel = -1)
        : m_set(s), m_memclass_sel(mclass_sel)
    {
    }
    MemBlockEnumToSet(blockid_set_t &s, MemoryClass mc)
        : m_set(s), m_memclass_sel(int(mc))
    {
    }
    virtual void supply_blocks_func(Tensor const *, int memclass,
                                    void *const *ptr, size_t num) override
    {
        if (m_memclass_sel >= 0 && memclass >= 0 && m_memclass_sel != memclass)
            return;
        for (size_t i = 0; i < num; i++) {
            if (ptr[i] != Allocator::vacant())
                m_set.emplace(ptr[i]);
        }
    }
};
...
class Interface {
  public:
    ...
    virtual void write_floats(void *ptr, const float *srcp,
                              size_t n) const noexcept;
    virtual void read_floats(float *dstp, const void *ptr,
                             size_t n) const noexcept;
    virtual inline float get_scale() const noexcept { return 1.0; }
    virtual inline float get_scale_recip() const noexcept { return 1.0; }
    virtual inline int32_t get_offset() const noexcept { return 0; }
    virtual void serialize(hnnx::Serializer &) const {};
    bool compare_equal(Interface const &)
            const noexcept; // implements operator == in general case
  protected:
...
class GenericAccessor : public GenericAccessorRO {
  public:
    GenericAccessor(void *data_in, const Interface &interface_in)
...
class NullInterface final : public Interface {
  public:
    ...
    virtual void write_float(void *ptr,
                             const float in) const noexcept override final
    {
    }
    virtual float read_float(const void *ptr) const noexcept override final
    {
        return 0.0f;
    }
    virtual size_t element_size() const noexcept override final { return 0; }
    virtual void write_floats(void *ptr, const float *srcp,
                              size_t n) const noexcept override final
    {
    }
    virtual void read_floats(float *dstp, const void *ptr,
                             size_t n) const noexcept override final;
  ...
  private:
    virtual bool
    compare_eq_same_type(Interface const *rhs) const noexcept override final
    {
        return true;
    }
...
template <typename T> class PlainInterface final : public Interface {
  public:
    ...
    static inline constexpr float convert_to_float(const T &in)
    {
        return float(in);
    }
    virtual void
    write_float(void *ptr,
                const float in) const noexcept override final; // inlined below
    virtual inline float
    read_float(const void *ptr) const noexcept override final
    {
        auto p = static_cast<const T *>(ptr);
        return convert_to_float(*p);
    }
    virtual size_t element_size() const noexcept override final
    {
        return sizeof(T);
    };
    virtual void write_floats(void *ptr, const float *srcp,
  ...
  private:
    ...
    class AcsrRO {
      ...
      public:
        using element_type = T;
        using AccessorRO = AcsrRO;
        AcsrRO(void const *data_in, PlainInterface const &)
    ...
    class Acsr {
      ...
      public:
        using element_type = T;
        using AccessorRO = AcsrRO;
        Acsr(void *data_in, PlainInterface const &)
...
template <typename T>
void PlainInterface<T>::write_floats(void *ptr, const float *srcp,
                                     size_t n) const noexcept
{
    T *dp = static_cast<T *>(ptr);
    for (int i = 0; i < (int)n; i++) {
        dp[i] = convert_from_float(srcp[i]);
    }
}
template <typename T>
void PlainInterface<T>::read_floats(float *dstp, const void *ptr,
                                    size_t n) const noexcept
{
    T const *sp = static_cast<T const *>(ptr);
    for (int i = 0; i < (int)n; i++) {
...
PlainInterface<float>::convert_from_float(const float &in)
{
    return in;
}
...
template <>
inline constexpr Float16
PlainInterface<Float16>::convert_from_float(const float &in)
{
    Float16 max_as_fp16 = std::numeric_limits<Float16>::max();
    float max_as_fp32 = static_cast<float>(max_as_fp16);
...
template <typename T>
inline void PlainInterface<T>::write_float(void *ptr,
                                           const float in) const noexcept
{
...
template <>
void PlainInterface<float>::write_floats(void *ptr, const float *srcp,
                                         size_t n) const noexcept;
template <>
void PlainInterface<float>::read_floats(float *dstp, const void *ptr,
                                        size_t n) const noexcept;
...
template <typename T> class ScaleOffsetInterface final : public Interface {
  ...
  public:
    ...
    using interface_other_type = ScaleOffsetInterface<TX>;
    template <typename TX> static inline constexpr T saturate(TX in)
    {
        return saturate_cast<T>(in);
    }
    inline constexpr T convert_from_float(float in) const
    {
        return saturate_round<T>(offset + in * scale_recip);
    }
    inline constexpr float convert_to_float(T in) const
    {
        if constexpr (sizeof(T) <= 2)
            return (float(int(in) - offset)) * scale;
        else
            return (float(in) - offset) * scale;
    }
    virtual inline void
    write_float(void *ptr, const float in) const noexcept override final
    {
        assert(ptr != nullptr);
        auto p = static_cast<T *>(ptr);
        *p = convert_from_float(in);
    ...
    virtual void read_floats(float *dstp, const void *ptr,
                             size_t n) const noexcept override final;
    virtual inline float get_scale() const noexcept override final
    {
        return scale;
    }
    virtual inline float get_scale_recip() const noexcept override final
    {
        return scale_recip;
    }
    virtual inline int32_t get_offset() const noexcept override final
    {
        return offset;
    }
    ScaleOffsetInterface(const int offset_, const float scale_)
        : offset(offset_), scale(scale_), scale_recip(1.0f / scale_)
    {
    }
    ScaleOffsetInterface(const OutputDef &def)
        : ScaleOffsetInterface(def.zero_offset, def.stepsize)
    {
        if (def.stepsize == 0.0f)
            debuglog("Oops: zero stepsize");
    }
    ScaleOffsetInterface() : ScaleOffsetInterface(0, 1.0f) {}
    ScaleOffsetInterface(hnnx::Deserializer &dctx)
        : offset(dctx.deserialize_uint32()), scale(dctx.deserialize_float()),
          scale_recip(1.0f / scale)
    {
    }
    virtual void serialize(hnnx::Serializer &sctx) const override
    {
        if (sctx.serialize_shared_obj(this)) {
            sctx.serialize_uint32(offset);
            sctx.serialize_float(scale);
        }
    }
    ...
    uint32_t interface_hash() const noexcept
    {
        return unsigned(offset) * 0x10661 ^
               (image_convert<unsigned, float>(scale) << 1);
    }
  ...
  private:
    ...
    class AcsrRO {
      ...
      public:
        ...
        {
            return interface.convert_to_float(val);
        }
        inline operator float() const { return as_float(); }
    ...
    class Acsr {
      ...
      public:
        ...
        inline float as_float() const
        {
            return interface.convert_to_float(*data);
        }
        inline operator float() const { return as_float(); }
        inline void set_float(float v)
        {
            *data = interface.convert_from_float(v);
        }
        inline void set_value(element_type v) { *data = v; }
        inline float operator=(float v)
        {
...
template <typename T>
void ScaleOffsetInterface<T>::write_floats(void *ptr, const float *srcp,
                                           size_t n) const noexcept
{
    ScaleOffsetInterface<T> intfc = *this;
    T *dp = static_cast<T *>(ptr);
    for (int i = 0; i < (int)n; i++) {
...
template <typename T>
void ScaleOffsetInterface<T>::read_floats(float *dstp, const void *ptr,
                                          size_t n) const noexcept
{
    ScaleOffsetInterface<T> intfc = *this;
    T const *sp = static_cast<T const *>(ptr);
...
template <typename T> struct hnnx::make_interface<ScaleOffsetInterface<T>> {
    ...
    static ScaleOffsetInterface<T> const *
    from_exemplar(Graph &, ScaleOffsetInterface<T> const &exemplar);
    static Interface const *from_odef(Graph &g, OutputDef const &odef)
    {
        // make an exemplar...
        ScaleOffsetInterface<T> exemplar(odef);
    ...
    static Interface const *from_deser(Deserializer &dctx)
    {
        ...
        Interface const **p = dctx.deserialize_shared_obj<Interface>();
        if (*p)
            return *p;
        // make an exemplar, by deserializing
        ScaleOffsetInterface<T> exemplar(dctx);
        // copy it to crate
        Interface const *new_p = to_crate(dctx.graph(), exemplar);
  ...
  protected:
    ...
    static ScaleOffsetInterface<T> const *
    to_crate(Graph &, ScaleOffsetInterface<T> const &exemplar);
};
...
template <typename T>
inline bool operator==(PlainInterface<T> const &, PlainInterface<T> const &)
{
    return true;
}
// we do need to compare ScaleOffsetInterface
template <typename T>
inline bool operator==(ScaleOffsetInterface<T> const &a,
                       ScaleOffsetInterface<T> const b)
{
    if (&a == &b)
...
//
template <typename TENSA, typename TENSB>
static constexpr bool tensors_compatible_layout()
{
    ...
    static_assert(
            std::is_same<TENSA, typename traitsA::implementation_type>::value);
    static_assert(
            std::is_same<TENSB, typename traitsB::implementation_type>::value);

    // must be the same layout_type and pad_type (and therefore the same rank)
    if constexpr (std::is_same<typename traitsA::layout_type,
                               typename traitsB::layout_type>::value &&
                  std::is_same<typename traitsA::pad_type,
                               typename traitsB::pad_type>::value) {
        // element type must be the same size
        using eltype_A = typename traitsA::element_type;
        if constexpr (sizeof(eltype_A) ==
                      sizeof(typename traitsB::element_type)) {
            // the A interface must be the same as the B (if we adapt B to A::element_type)
            using adapted_B_interface = typename traitsB::interface_type::
                    template interface_other_type<eltype_A>;
            if constexpr (std::is_same<typename traitsA::interface_type,
                                       adapted_B_interface>::value) {
                static_assert(
                        sizeof(TENSA) ==
                        sizeof(TENSB)); // make sure we didn't miss anything
                return true;
            }
        }
    }
...
template <typename TENSA, typename TENSB>
static constexpr bool tensors_compatible_implementation()
{
    return tensors_compatible_layout<
...
class Tensor {
  public:
    ...
    GenericAccessor generic_accessor(void *p) noexcept
    {
        return GenericAccessor(p, interface());
    }
    GenericAccessorRO generic_accessor_ro(void const *p) const noexcept
    {
    ...
    rank() const noexcept = 0; // What's the rank of this tensor?
    virtual const size_t dim(size_t index)
            const noexcept = 0; // What's the length of some dimension?
    virtual std::pair<size_t const *, size_t>
    get_dims() const noexcept = 0; // return rank, and address of dims[0..n-1]
    // this is the first virtual method defined externally.
    virtual uint32_t
    find_content_hash() const noexcept; // find 'content hash' of the data.
  ...
  protected:
    ...
    virtual void *element_addr(size_t rank,
                               SIdx const coords_in[]) const noexcept = 0;

  public:
    // element_ptr on insufficiently specialized class gives the result as a void *.
    inline void const *element_ptr(size_t rank,
                                   const SIdx coords[]) const ALWAYS_INLINE
    {
        return (void const *)element_addr(rank, coords);
    }
    inline void *element_ptr(size_t rank, const SIdx coords[]) ALWAYS_INLINE
    {
    ...
    std::tuple<size_t, size_t, size_t, size_t> get_dims_4() const
    {
        ...
        if (n != 4)
            throw std::runtime_error("rank not 4");
    ...
    std::tuple<size_t, size_t> get_dims_1_2() const
    {
        ...
        std::tie(ptr, n) = get_dims(); // virtual call
        if (n < 3)
            throw std::runtime_error("rank not >=3");
    ...
    inline GenericAccessorRO operator()(ind_types... inds) const
    {
        const SIdx indarr[] = {static_cast<SIdx>(inds)...};
        return this->generic_accessor_ro(
                element_addr(sizeof...(ind_types), indarr));
    }
    template <typename... ind_types>
    inline GenericAccessor operator()(ind_types... inds)
    {
        const SIdx indarr[] = {static_cast<SIdx>(inds)...};
        return this->generic_accessor(
                element_addr(sizeof...(ind_types), indarr));
    }
    /* Use single uint32 to store tensor format
	 * Bits 7-0 type; Bits 15-8 Rank; Bits 23-16 memory class
	 * Bits 27-24 is padded Bits 31-28 is continuous
	 */
    static inline constexpr uint32_t
    pack_serialize_data(DType type, uint32_t rank, MemoryClass mclass,
                        uint32_t is_padded, uint32_t is_continuous)
    {
        uint32_t tinfo = 0;
        tinfo |= static_cast<uint32_t>(type);
        tinfo |= (rank & 0xff) << 8;
        tinfo |= (static_cast<uint32_t>(mclass) & 0xff) << 16;
        tinfo |= (is_padded & 0xf) << 4;
        tinfo |= (is_continuous & 0xf) << 4;
        return tinfo;
    }

    virtual const uint32_t get_tensor_info() const noexcept { return 0; }
    // returns false if the dims are the same; true if different, or maybe different.
    virtual bool
    set_dims(const size_t dims[]) = 0; // Set the shape of the tensor
    virtual bool set_dims(
            const Tensor &
                    prototype) = 0; // Set the shape of the tensor same as another.

    inline void allocate(hnnx::Allocator &allocator, unsigned options = 0)
    {
    ...
    void const *raw_data_const() const noexcept
    {
    ...
    void get_memory_blocks(hnnx::blockid_set_t &blocklist,
                           int mc_sel = -1) const;
    inline void get_memory_blocks(hnnx::blockid_set_t &blocklist,
                                  MemoryClass mc) const
    {
        get_memory_blocks(blocklist, int(mc));
    }
    // return the set of memory blocks
    hnnx::blockid_set_t get_memory_blocks(int mc_sel = -1) const;
    inline hnnx::blockid_set_t get_memory_blocks(MemoryClass mc) const
    {
    ...
    template <typename ENFUNC>
    inline void enum_memory_blocks_withfunc(ENFUNC &&ef) const
    {
    ...
    template <typename REPLFUNC>
    inline void replace_memory_blocks_withfunc(REPLFUNC &&rf)
    {
        hnnx::MemBlockReplBlockWrapper<std::remove_reference_t<REPLFUNC>>
                enumer(std::forward<REPLFUNC>(rf));
        this->enum_memory_blocks(enumer);
    }
    // this is passed a map<void*,void*> or any similar type with find() and end(),
    // and uses it to edit the blocks in the tensor.
    template <typename MAPTYPE>
    inline void replace_memory_blocks_withmap(MAPTYPE const &map)
    {
    ...
    virtual void serialize(hnnx::Serializer &sctx) const;
    // The same tensor in the same layout, but with persistent storage.

    std::unique_ptr<Tensor> persistent_clone(hnnx::Allocator *allocator,
                                             bool zoneb = false) const;
    // same thing, but does refcounts in 'zone B'
    inline std::unique_ptr<Tensor>
    persistent_clone_Op(hnnx::Allocator *allocator) const
    {
    ...
    void persistent_decref(hnnx::Allocator *allocator,
                           bool zoneb = false) const;
    // same thing, but does refcounts in 'zone B'
    inline void persistent_decref_Op(hnnx::Allocator *allocator) const
    {
    ...
    std::unique_ptr<Tensor> reallocate_clone(hnnx::Allocator *allocator,
    ...
    virtual void const *read_tile(unsigned flags, void *buffer, size_t b, int h,
                                  int w, int d) const;
    virtual void write_tile(unsigned flags, void const *buffer, size_t b, int h,
                            int w, int d);
    enum {
        tile_8bit =
        ...
        tile_fast =
                16, // set only when one of the XXbit is set, and the support is vector accelerated.
        tile_direct =
                32 // set only when when 'fast' is set, and a direct mapping is possible
    };

    virtual unsigned tile_support_bits() const;
    inline bool tile_support() const
    {
        return (tile_support_bits() & tile_any) != 0;
    }
    inline bool tile_support_fast() const
    {
        return (tile_support_bits() & tile_fast) != 0;
    }
    inline bool tile_support_direct() const
    {
    ...
    void *write_tile_strategy(unsigned flags, void *buffer, size_t b, int h,
                              int w, int d);

    static uint32_t content_hash_data(void const *, size_t nbytes,
                                      bool is_float) noexcept;
    static uint32_t content_hash_data_indirect(uint32_t inhash, void **blocks,
                                               unsigned nblocks,
                                               size_t blockbytes,
                                               bool is_float) noexcept;

    static uint32_t build_hash(size_t const *dims, int n,
    ...
    struct tensor_blockinfo {
        ...
        bool is_chunked; // inidicates that the layout is chunked;
        bool is_container; // set if the tensor is a 'container' of other tensors.
                // if so, the other info is possibly meaningless.
        void setup(DType dt = DType::UNKNOWN,
                   MemoryClass mc = MemoryClass::Default)
        {
            blkptrs = nullptr;
            shapepp = nullptr;
            nblocks = 0;
    ...
    inline void get_tensor_blockinfo(tensor_blockinfo *infop) const
    {
        clone_util(nullptr, nullptr, infop);
    }

  protected:
    virtual void allocate_func(hnnx::Allocator &allocator,
                               unsigned options) = 0;
    ...
    //  has no blocks).
    //
    // 'container' tensors don't support any clone method except duplicate_clone.
    // For these types, if both tensp and infop are not null, the clone operation is declined
    // (i.e. no duplicate is created, *tensp is unchanged; infop->is_container is set true,
    // and null is returned. You can still get a duplicate clone by calling with infop ==null
    // (but it might not really do what you want).
    // To clarify, if infop != null, infop->is_container is always set to reflect whether the
    // the type is a container (and if it is, maybe nothing else in infop is reliable).
    //
    // Note: allocator may be null if tensp is null.

    virtual void **clone_util(hnnx::Allocator *allocator,
                              std::unique_ptr<Tensor> *tensp,
...
class FakeTensor : public Tensor {
  ...
  protected:
    ...
    virtual void *element_addr(size_t rank,
                               SIdx const coords_in[]) const noexcept override;
  ...
  public:
    ...
    virtual std::pair<size_t const *, size_t>
    get_dims() const noexcept override; //->{null,0}
    virtual bool set_dims(const size_t dims[]) override; // -> false
    virtual bool set_dims(const Tensor &prototype) override; // ->false
    virtual void
    enum_memory_blocks(hnnx::MemBlockEnumerator &) const override; // nothing

    virtual DTypeScaleOff get_dtype_intfc()
...
template <unsigned TRank> class RankedTensor : public Tensor {
  public:
    ...
    RankedTensor(const Op *producer_in, hnnx::Deserializer &dctx)
        : Tensor(producer_in, dctx)
    {
    }
    RankedTensor(const RankedTensor &old, hnnx::Allocator *allocator,
                 clone_mode cmode)
        : Tensor(old, allocator, cmode)
    {
    }
    static constexpr auto Rank = TRank;
    virtual inline const size_t rank() const noexcept override final
    {
        return Rank;
    }
    template <typename... ind_types>
    inline GenericAccessorRO operator()(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const SIdx indarr[] = {static_cast<SIdx>(inds)...};
...
template <unsigned TRank> class TensorShape : public RankedTensor<TRank> {
  ...
  protected:
    ...
    virtual void *
    element_addr(size_t rank,
                 SIdx const coords_in[]) const noexcept override final
    {
        return nullptr;
    }
    virtual Interface const &interface() const noexcept override
  ...
  public:
    const char *true_name() const override
    {
        return type_name<TensorShape<TRank>>();
    };
    ...
    virtual const size_t dim(size_t index) const noexcept override final
    {
        return shape[index];
    }
    ...
    TensorShape(const Op *producer_in, const OutputDef &def, Graph &graph_in)
        : Parent(producer_in),
          shape(hnnx::ptr_to_stdarray<Rank, size_t>(&def.max_sizes[0]))
    {
    }
    TensorShape(const Op *producer_in, hnnx::Deserializer &dctx)
        : Parent(producer_in, dctx),
          shape(dctx.deserialize_uint32_array_sizet<Rank>())
    {
    }
    TensorShape(const TensorShape &old, hnnx::Allocator *allocator,
                Tensor::clone_mode cmode)
        : Parent(old, allocator, cmode), shape(old.shape)
    {
    }

    virtual void enum_memory_blocks(hnnx::MemBlockEnumerator &) const override
    {
        return;
    }
    ...
    virtual void serialize(hnnx::Serializer &sctx) const override
    {
        Parent::serialize(sctx);
        for (int i = 0; i < Rank; i++) {
            sctx.serialize_uint32(shape[i]);
        }
    }
    ...
    {
        if (tensp)
            *tensp = std::make_unique<TensorShape>(
                    *this, allocator, Tensor::clone_mode::duplicate);
        if (infop)
            infop->setup();
        return nullptr;
    }
  ...
  protected:
    virtual int compare_sametype(const Tensor *rhs_in) const override
    {
        ...
        for (int i = 0; i < Rank; i++) {
            int dimdiff = this->shape[i] - rhs->shape[i];
            if (dimdiff != 0)
                return dimdiff;
        }
        return 0;
...
template <DType DT> class TensorSclrDT : public Tensor {
  protected:
    ...
    using Interface_t =
            std::conditional_t<dtype_traits<DT>::is_quant,
                               ScaleOffsetInterface<T>, PlainInterface<T>>;
  ...
  public:
    ...
    inline float interface_scale_recip() const
    {
        return interface_inst.get_scale_recip();
    }
    inline int32_t interface_offset() const
    {
        return interface_inst.get_offset();
    }
  ...
  public:
    const char *true_name() const override
    {
        return type_name<TensorSclrDT<DT>>();
    };
    ...
    virtual const size_t rank() const noexcept override
    {
        return 0;
    } // What's the rank of this tensor?
    virtual const size_t dim(size_t index) const noexcept override
    {
        return 1;
    ...
    TensorSclrDT(const Op *producer_in, T value_in)
        : Tensor(producer_in), value(value_in)
    {
        static_assert(!dtype_traits<DT>::is_quant,
                      "FIXME: need different constructor");
    }
    TensorSclrDT(const Op *producer_in, hnnx::Deserializer &dctx)
        : Tensor(producer_in, dctx), interface_inst(dctx),
          value(dctx.deserialize_type<T>())
    {
    }
    TensorSclrDT(const TensorSclrDT &old, hnnx::Allocator *allocator,
                 clone_mode cmode)
        : Tensor(old, allocator, cmode), interface_inst(old.interface_inst),
          value(old.value)
    {
    }

    template <typename... ind_types>
    inline const Const_Accessor_t operator()(ind_types... inds) const
    {
        return Const_Accessor_t((void *)&value, this->interface());
    }
    template <typename... ind_types>
    inline Accessor_t operator()(ind_types... inds)
    {
    ...
    {
        Tensor::serialize(sctx);
        interface().serialize(sctx);
        sctx.serialize_type<T>(value);
    }
    virtual void **clone_util(hnnx::Allocator *allocator,
                              std::unique_ptr<Tensor> *tensp,
                              Tensor::tensor_blockinfo *infop) const override
    {
        if (tensp)
            *tensp = std::make_unique<TensorSclrDT>(*this, allocator,
                                                    clone_mode::duplicate);
        if (infop)
            infop->setup(DT);
        return nullptr;
    }
  ...
  protected:
    ...
    virtual uint32_t find_content_hash() const noexcept override
    {
        uint32_t h = interface().interface_hash() ^
                     mulu32_modular(unsigned(DT), 0x107301);
        return mulu32_modular(h, 0x104301) ^
               content_hash_data(&this->value, sizeof(T),
                                 dtype_traits<DT>::is_float);
    }
};
...
struct layout_mem_contig {
    ...
    inline layout_mem_contig(Shape_t const *shp, Graph &graph_in)
        : bulk_data(){};

    // duplicate clone from another
    inline layout_mem_contig(Shape_t const *shp, layout_mem_contig const &other,
    ...
    inline void *raw_data() const noexcept ALWAYS_INLINE
    {
    ...
    void *element_addr(Shape_t const *shp, size_t rank,
                       SIdx const coords_in[]) const noexcept ALWAYS_INLINE
    {
        //assert(rank == Rank);
        const std::array<size_t, Rank> padded_coords = pad.pad_coords(
                hnnx::ptr_to_stdarray<Rank, SIdx>(&coords_in[0]), shp->pad);
        size_t offset = layout.linear_offset(padded_coords, shp->max_dims);
        return (void *)&bulk_data[offset];
    }

    // get pointer to block table, and length
    inline void **get_block_list_ptr() const ALWAYS_INLINE
    {
        return (void **)&bulk_data;
    }
    ...
    get_elements_per_block(Shape_t const *shp) ALWAYS_INLINE
    {
        return std::accumulate(shp->max_dims.cbegin(), shp->max_dims.cend(), 1,
    ...
    inline storage_type **block_ptr_addr(Shape_t const *shape,
                                         std::array<SIdx, Rank> coords) const
    {
        return &bulk_data;
    }
    // dummy for this
    inline void realloc_blocktab(hnnx::Allocator *alloc,
                                 Shape_t const *old_shape,
    ...
    // serialize blocks
    void serialize_blocks(Shape_t const *shp, hnnx::Serializer &sctx) const
    {
        sctx.allocator->serialize_blocks(
                sctx, (void const *const *)&bulk_data,
                sizeof(storage_type) * get_elements_per_block(shp), 1);
        sctx.serialize_uint32(TENSOR_MAGIC);
    }
    // compare memory (raw compare)
    int compare_memory(Shape_t const *shp, layout_mem_contig const &rhs) const
    {
        size_t len = get_elements_per_block(shp) * sizeof(storage_type);
        return memcmp(bulk_data, rhs.bulk_data, len);
    }
    // find content hash of memory.
    //
    uint32_t find_content_hash(Shape_t const *shp, uint32_t oldhash,
                               bool is_float) const
    {
        size_t len = get_elements_per_block(shp) * sizeof(storage_type);
        return mulu32_modular(oldhash, 0x223131) ^
               Tensor::content_hash_data(bulk_data, len, is_float);
    }
};
...
inline int compare_indirect_blocks(void **ptr_a, void **ptr_b, size_t nblocks,
                                   size_t blocklen)
{
    for (size_t i = 0; i < nblocks; i++) {
        int cmp = memcmp(ptr_a[i], ptr_b[i], blocklen);
        if (cmp != 0)
            return cmp;
    }
    return 0;
...
struct layout_mem_indirect {
    ...
    // duplicate clone from another
    layout_mem_indirect(Shape_t const *shp, layout_mem_indirect const &other,
                        hnnx::Allocator *alloc, Tensor::clone_mode cmode)
        : blocktab()
    {
        unsigned nblocks = layout.num_blocks(shp->max_dims);
        blocktab = (storage_type **)indirect_layout_mem::make_blocktab(
                nblocks, alloc->graph);
        std::memcpy(blocktab, other.blocktab, sizeof(void *) * nblocks);
    }
    // construct from deserialize
    layout_mem_indirect(Shape_t const *shp, hnnx::Deserializer &dctx)
        : blocktab()
    {
        unsigned nblocks = layout.num_blocks(shp->max_dims);
        blocktab = (storage_type **)indirect_layout_mem::make_blocktab(
                nblocks, dctx.graph());
        dctx.allocator->deserialize_blocks(dctx, (void const **)blocktab,
                                           nblocks);
        DESERIALIZE_ASSERT_UINT32(dctx, TENSOR_MAGIC);
    }
    ...
    {
        assert(rank == Rank);
        std::array<size_t, Rank> padded_coords = pad.pad_coords(
                hnnx::ptr_to_stdarray<Rank, SIdx>(&coords_in[0]), shp->pad);
        size_t block_offset = layout.chunk_offset(padded_coords, shp->max_dims);
        size_t block_idx = layout.chunk_index(padded_coords, shp->max_dims);
        return (void *)&blocktab[block_idx][block_offset];
    }
    // get pointer to block table, and length
    inline void **get_block_list_ptr() const ALWAYS_INLINE
    {
        return (void **)blocktab;
    }
    static inline size_t get_block_list_len(Shape_t const *shp) ALWAYS_INLINE
    {
    ...
    // block size for allocation
    inline static size_t get_elements_per_block(Shape_t const *) ALWAYS_INLINE
    {
        return layout.chunk_total;
    }
    // find the address of the block pointer containing the specified coords.
    inline storage_type **block_ptr_addr(Shape_t const *shape,
                                         std::array<SIdx, Rank> coords) const
    {
        std::array<size_t, Rank> padded_coords =
                pad.pad_coords(coords, shape->pad);
        size_t block_idx = layout.chunk_index(padded_coords, shape->max_dims);
    ...
    inline void realloc_blocktab(hnnx::Allocator *alloc,
                                 ...
                                 Shape_t const *new_shape)
    {
        unsigned nblocks = layout.num_blocks(new_shape->max_dims);
        if (old_shape != new_shape) {
            unsigned old_nblocks = layout.num_blocks(old_shape->max_dims);
    ...
    void serialize_blocks(Shape_t const *shp, hnnx::Serializer &sctx) const
    {
        auto nblocks = layout.num_blocks(shp->max_dims);
        sctx.allocator->serialize_blocks(
                sctx, (void *const *)blocktab,
                sizeof(storage_type) * layout.chunk_total, nblocks);
        sctx.serialize_uint32(TENSOR_MAGIC);
    }
    // compare memory (raw compare)
    int compare_memory(Shape_t const *shp, layout_mem_indirect const &rhs) const
    {
        size_t nblocks = layout.num_blocks(shp->max_dims);
        size_t blocklen = sizeof(storage_type) * layout.chunk_total;
        return indirect_layout_mem::compare_indirect_blocks(
                (void **)blocktab, (void **)rhs.blocktab, nblocks, blocklen);
    }
    // find content hash of memory.
    //
    uint32_t find_content_hash(Shape_t const *shp, uint32_t oldhash,
                               bool is_float) const
    {
        size_t nblocks = layout.num_blocks(shp->max_dims);
        size_t blocklen = sizeof(storage_type) * layout.chunk_total;
        return Tensor::content_hash_data_indirect(oldhash, (void **)blocktab,
                                                  nblocks, blocklen, is_float);
    }
};
///////////////////////////////////////////
...
template <typename Linfo> class BlockTableAccessor {
  protected:
    ...
    std::array<size_t, Rank>
            blkstrides; // 'strides' (note stride for dim i is blkstrides[i+1];
  ...
  public:
        ...
        : blktab(tens.blocktab_ptr())
    {
        ...
        for (int i = Rank - 1; i >= 0; --i) {
            ...
            size_t blkdim = shp.max_dims[i] / blk;
            allprod *= blkdim;
            blkdims[i] = blkdim;
            margin[i] = shp.pad[i];
    ...
    inline size_t blocktab_blocksize_bytes() const
    {
        return TLayout::chunk_total * sizeof(storage_type);
    };

    inline size_t blocktab_dim(int i) const { return blkdims[i]; }
    inline size_t blocktab_dim_stride(int i) const
    {
    ...
    template <typename... ind_types>
    inline pointer_type *block_ptr_address(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
    ...
    template <typename... ind_types>
    inline pointer_type &block_ptr(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    //
    template <typename... ind_types>
    inline pointer_type &blocktab(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    {
        if (!same_table_shape(other))
            return false;
        for (int i = 0; i < Rank; i++)
            if (margin[i] != other.margin[i])
                return false;
        return true;
    }
...
class LayoutTensor : public RankedTensor<Linfo::Rank> {
  protected:
    ...
    static constexpr bool is_padded =
            !std::is_same<Pad_t, NoPadding<Rank>>::value;

    static_assert(!(is_indirect && !is_chunked),
                  "non-chunked layouts can't be indirect");

    Interface const *const
            interface_ptr; // pointer to shared instance of Interface subclass.
    using Shape_t = Shape<Rank>;
  ...
  protected: // interface, then shape, then mem
    using layout_mem_t = std::conditional_t<
            is_indirect, layout_mem_indirect<storage_type, TLayout, Pad_t>,
  ...
  protected:
    ...
    [[gnu::noinline]] LayoutTensor(
            const Op *producer_in, const OutputDef &def, Graph &graph_in,
        ...
        : BaseRT(producer_in), interface_ptr((*ifc_maker)(graph_in, def)),
          shape(Shape_t::canonical_shape(
                  graph_in,
                  Shape_t(hnnx::ptr_to_stdarray<Rank, size_t>(
                                  &def.max_sizes[0]),
                          mem.layout.pad(hnnx::ptr_to_stdarray<Rank, size_t>(
                                  &def.max_sizes[0]))))),
          mem(shape, graph_in)
    {
    }
    [[gnu::noinline]] LayoutTensor(
            const Op *producer_in, hnnx::Deserializer &dctx,
            Interface const *(*ifc_deser_fp)(hnnx::Deserializer &))
        : BaseRT(producer_in, dctx), interface_ptr((*ifc_deser_fp)(dctx)),
          shape(Shape_t::deserialize(dctx)), mem(shape, dctx)
    {
    }
    // clone ctor.
    [[gnu::noinline]] LayoutTensor(const LayoutTensor &old,
                                   hnnx::Allocator *allocator,
                                   Tensor::clone_mode cmode)
        : BaseRT(old, allocator, cmode), interface_ptr(old.interface_ptr),
          shape(old.shape), mem(shape, old.mem, allocator, cmode)
    {
    }
  ...
  public:
    virtual void serialize(hnnx::Serializer &sctx) const override
    {
        assert("can't serialize this directly!");
    }

    virtual const inline size_t dim(size_t index) const noexcept override final
    {
    ...
    inline float interface_scale_recip() const
    {
        return interface().get_scale_recip();
    }
    ...
    inline std::conditional_t<is_indirect, void, storage_type *const &>
    data_ptr() const
    {
        if constexpr (!is_indirect) {
            return mem.bulk_data;
    ...
    // block table access
    inline storage_type **blocktab_ptr() const
    {
    ...
    inline size_t blocktab_blocksize() const
    {
        return mem.get_elements_per_block(shape);
    }
    inline size_t blocktab_blocksize_bytes() const
    {
    ...
    {
        size_t total_elements = std::accumulate(shape->max_dims.cbegin(),
                                                shape->max_dims.cend(), 1,
                                                std::multiplies<size_t>());
        return total_elements;
    }
    virtual void *raw_data() noexcept override final { return mem.raw_data(); }
    virtual void set_raw_data_despite_danger(void *buffer) override final
    {
    ...
    void change_pad(std::array<size_t, Rank> const &new_pad,
                    hnnx::Allocator &allocator)
    {
        ...
        newshape.max_dims =
                layout.pad(pad.pad_coords(newshape.dims, newshape.pad));
        // nake a persistent copy of new shape
        Shape_t const *new_shape_p =
                Shape_t::canonical_shape(allocator.graph, newshape);
    ...
    template <typename... ind_types>
    inline storage_type const *const *block_ptr_address(ind_types... inds) const
    {
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
        return mem.block_ptr_addr(shape, coords);
    }
    template <typename... ind_types>
    inline storage_type *const *block_ptr_address(ind_types... inds)
    {
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
        return mem.block_ptr_addr(shape, coords);
    }
    template <typename... ind_types>
    inline storage_type const *block_ptr(ind_types... inds) const
    {
        return *block_ptr_address(inds...);
    }
    template <typename... ind_types>
    inline storage_type *block_ptr(ind_types... inds)
    {
        return *block_ptr_address(inds...);
    }

    std::conditional_t<is_indirect, BlockTableAccessor<Linfo>, void>
    blocktable_accessor() const
    {
        if constexpr (is_indirect) {
    ...
    std::conditional_t<is_indirect, std::array<size_t, Rank>, void>
    tile_strides() const
    {
        if constexpr (is_indirect) {
            ...
            for (int i = Rank - 2; i >= 0; i--) {
                ret[i] = ret[i + 1] *
                         (shape->max_dims[i + 1] / layout.ChunkSizes[i + 1]);
            }
            return ret;
        }
    }
    ...
    template <typename... ind_types>
    inline storage_type const *get_raw_addr(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline storage_type *get_raw_addr(ind_types... inds)
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline storage_type const &get_raw(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline storage_type &get_raw(ind_types... inds)
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    virtual void const *read_tile(unsigned flags, void *buffer, size_t b, int h,
                                  int w, int d) const override;
    virtual void write_tile(unsigned flags, void const *buffer, size_t b, int h,
                            int w, int d) override;
    ...
    [[gnu::noinline]] virtual uint64_t get_checksum() const override
    {
        ...
        if constexpr (Rank == 4) {
            ...
            if (batch && heights && width && depth) {
                storage_type x0 =
                        *(storage_type const *)this->get_raw_addr(0, 0, 0, 0);
                for (size_t b = 0; b < batch; b++) {
                    for (size_t h = 0; h < heights; h++) {
                        for (size_t w = 0; w < width; w++) {
                            for (size_t d = 0; d < depth; d++) {
                                storage_type x =
                                        *(storage_type const *)this
                                                 ->get_raw_addr(b, h, w, d);
                                x ^= x0;
                                union {
                                    storage_type as_x;
                                    uint8_t as_byte[sizeof(storage_type)];
                                } uu = {x};
                                chk = hnnx::checksum_bytes(
                                        chk, uu.as_byte, sizeof(storage_type));
                            }
                        }
                    }
                }
  ...
  protected:
    ...
    virtual void *element_addr(size_t rank, const SIdx coords_in[])
            const noexcept final override ALWAYS_INLINE
    {
    ...
    [[gnu::noinline]] int compare_sametype_layout(LayoutTensor const *rhs) const
    {
        if (shape->dims != rhs->shape->dims)
            return (shape->dims < rhs->shape->dims) ? -1 : 1;
        if (is_padded) {
            if (shape->max_dims != rhs->shape->max_dims)
                return (shape->max_dims < rhs->shape->max_dims) ? -1 : 1;
    ...
    // This is called from ConcreteTensor::serialize to fully serialize that class, since only
    // the interface is different among them, and that's got a virtual method to serialize.
    // We could just define this as the serialize() method here; but this trick avoids
    // accidentally serializing a LayoutTensor which isn't a ConcreteTensor for some reason.
    [[gnu::noinline]] void serialize_layout(hnnx::Serializer &sctx) const
    {
        BaseRT::serialize(sctx);
        interface().serialize(sctx); // this is a virtual method.
        shape->serialize(sctx);
        // now the block table.
        mem.serialize_blocks(shape, sctx);
    }
    // allocation and enumeration.
    [[gnu::noinline]] void allocate_layout(hnnx::Allocator &allocator,
                                           unsigned options, MemoryClass mclass)
    {
        // get the pointer to block table; and number of entries in it.
        void **blocktab = this->mem.get_block_list_ptr();
        size_t nblocks = this->mem.get_block_list_len(this->shape);
        size_t blocksize = sizeof(storage_type) *
                           this->mem.get_elements_per_block(this->shape);
        size_t align = traits::is_indirect
                               ? blocksize
                               : std::min(size_t(256), sizeof(storage_type));

        allocator.allocate_n(blocktab, // pointer to pointers,
                             nblocks, // number of pointers
                             blocksize, align, mclass, options);
    }
    [[gnu::noinline]] void
    enum_memory_blocks_layout(hnnx::MemBlockEnumerator &en,
    ...
    find_content_hash_layout(uint32_t hash_in, bool is_float) const noexcept
    {
...
class ConcreteTensor : public LayoutTensor<typename Tinfo::Lconfig> {
  protected:
    ...
    static_assert(
            Rank == BaseLayout::Rank &&
                    is_indirect == BaseLayout::traits::is_indirect &&
                    std::is_same<
                            Layout_t,
                            typename BaseLayout::traits::layout_type>::value &&
                    std::is_same<Pad_t,
                                 typename BaseLayout::traits::pad_type>::value,
  ...
  public:
    ...
    //   from the output def.
    ConcreteTensor(const Op *producer_in, const OutputDef &def, Graph &graph_in)
        : BaseLayout(producer_in, def, graph_in,
                     hnnx::make_interface<Interface_t>::from_odef)
    {
    }
    ConcreteTensor(const Op *producer_in, const OutputDef &def, Graph &graph_in,
    ...
    ConcreteTensor(const Op *producer_in, hnnx::Deserializer &dctx)
        : BaseLayout(producer_in, dctx,
                     &hnnx::make_interface<Interface_t>::from_deser)
    {
    }
    //    - 'clone duplicate' of the given tensor. Note that cmode is ignored.
    ConcreteTensor(const ConcreteTensor &old, hnnx::Allocator *allocator,
                   Tensor::clone_mode cmode)
    ...
    virtual void serialize(hnnx::Serializer &sctx) const override
    {
        // all done in the base class.
        BaseLayout::serialize_layout(sctx);
    }
    virtual DTypeScaleOff get_dtype_intfc() const noexcept override
    {
        return DTypeScaleOff(dtype, interface());
    ...
    element_ptr(size_t rank, const SIdx coords[]) const ALWAYS_INLINE
    {
        return (element_type const *)this->element_addr(rank, coords);
    }
    inline element_type *element_ptr(size_t rank,
                                     const SIdx coords[]) ALWAYS_INLINE
    {
    ...
    inline std::conditional_t<is_indirect, void, element_type *const &>
    data_ptr() const
    {
        if constexpr (!is_indirect) {
    ...
    // block table access
    inline element_type **blocktab_ptr() const
    {
        return (element_type **)this->mem.get_block_list_ptr();
    }
    inline element_type *&blocktab_at(size_t i)
    {
    ...
    template <typename... ind_types>
    inline element_type const *const *block_ptr_address(ind_types... inds) const
    {
        return (element_type const *const *)BaseLayout::block_ptr_address(
                inds...);
    };
    template <typename... ind_types>
    inline element_type *const *block_ptr_address(ind_types... inds)
    {
        return (element_type *const *)BaseLayout::block_ptr_address(inds...);
    };
    template <typename... ind_types>
    inline element_type const *block_ptr(ind_types... inds) const
    {
        return *block_ptr_address(inds...);
    }
    template <typename... ind_types>
    inline element_type *block_ptr(ind_types... inds)
    {
        return *block_ptr_address(inds...);
    }
    ...
    //
    template <typename... ind_types>
    inline Const_Accessor_t operator()(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {static_cast<SIdx>(inds)...};
        return Const_Accessor_t(this->element_addr(Rank, coords.data()),
                                interface());
    }
    template <typename... ind_types>
    inline Accessor_t operator()(ind_types... inds)
    ...
    template <typename... ind_types>
    inline element_type const &get_raw(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline element_type &get_raw(ind_types... inds)
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline element_type const *get_raw_addr(ind_types... inds) const
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    template <typename... ind_types>
    inline element_type *get_raw_addr(ind_types... inds)
    {
        static_assert(Rank == (sizeof...(ind_types)),
                      "# of coords must match Rank");
        const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    ...
    virtual const uint32_t get_tensor_info() const noexcept override
    {
        static constexpr uint32_t info = Tensor::pack_serialize_data(
                traits::dtype, Rank, traits::memclass, BaseLayout::is_padded,
                !traits::is_indirect);
        return info;
    }
    // allocation and enumeration.
    ...
    {
        uint32_t h = interface().interface_hash() ^
                     mulu32_modular(unsigned(dtype), 0x107301);
  ...
  protected:
    virtual int compare_sametype(const Tensor *rhs_in) const override
    {
        ...
        int icmp = interface().compare(rhs->interface());
        if (icmp != 0)
            return icmp;
                              ...
                              Tensor::tensor_blockinfo *infop) const override
    {
        ...
        if (tensp) {
            *tensp = std::make_unique<ConcreteTensor>(
                    *this, allocator, Tensor::clone_mode::duplicate);
        ...
        if (infop) {
            ...
            infop->shapepp =
                    (hnnx::ShapeFlags *const *)&(newtens ? newtens : this)
                            ->shape;
            infop->nblocks = this->mem.get_block_list_len(this->shape);
            infop->blocksize = sizeof(element_type) *
                               this->mem.get_elements_per_block(this->shape);
...
template <typename T> class IterableTensor {
  ...
  public:
    ...
    inline IterableTensor(reference t, std::array<size_t, 4> inc)
        : myTensor(&t), myCTensor(const_cast<const_pointer>(&t)),
          increments(inc), is_const(false)
    {
    ...
    inline auto operator()(size_t b, size_t h, size_t w, size_t d)
    {
    ...
    {
        return (this->myCTensor == it.myCTensor) &&
               (this->increments == it.increments);
    }
    inline bool operator!=(const IterableTensor<T> &it) const
    {
        return !(*this == it);
    ...
    inline iterator begin(std::array<size_t, 4> start)
    {
        return iterator(*this, start);
    }
    inline const_iterator begin(std::array<size_t, 4> start) const
    {
        return const_iterator(*this, start);
    }
    inline iterator end()
    ...
    inline iterator end(std::array<size_t, 4> end)
    {
        return iterator(*this, end);
    }
    inline const_iterator end(std::array<size_t, 4> end) const
    {
        return const_iterator(*this, end);
    }
...
template <typename T> class TensorIter {
  private:
    ...
    {
        size_t inc = myITensor.increments[dim];
        if (inc) {
            size_t loc = location[dim];
  ...
  protected:
    inline TensorIter(const TensorIter<T> &to_copy)
        : myITensor(to_copy.myITensor), location(to_copy.location)
    {
    }

  public:
    inline TensorIter(IterableTensor<T> &it, std::array<size_t, 4> loc)
    ...
    {
        TensorIter<T> clone = TensorIter<T>(*this);
        incrementLocation();
        return clone;
    }
...
template <typename T> class TensorCIter {
  ...
  public:
    inline TensorCIter(const IterableTensor<T> &it, std::array<size_t, 4> loc)
        : myITensor(it), location(loc)
    {
    }
    ...
    inline operator float() const
    {
        return myITensor.read(location[0], location[1], location[2],
                              location[3]);
    }
...
#define DEFINE_TYPENAMES(TYPE, NAME)                                           \
    ...
    DEFINE_TYPENAME(TYPE &, NAME);                                             \
    DEFINE_TYPENAME(const TYPE &, NAME);                                       \
    DEFINE_TYPENAME(Vector<const TYPE *> &, NAME);                             \
    DEFINE_TYPENAME(const Vector<const TYPE *> &, NAME);
...
#define TENSORDEF_MC(NAME, LAYOUTNAME, DTYPE, MCLASS, ENCODENAME)              \
    ...
    struct NAME {                                                              \
        ...
        static_assert(sizeof(element_type) == sizeof(storage_type),            \
                      "layout has wrong element size");                        \
        using Interface_t =                                                    \
                std::conditional_t<dtype_traits<DTYPE>::is_quant,              \
...
TENSORDEF_MC(QuantUint8_5D_TCM, Flat5D_8, DType::QUInt8, MemoryClass::TCM,
             "F5B")
...
TENSORDEF_MC(QUint8Crouton_TCM, Crouton_8, DType::QUInt8, MemoryClass::TCM,
             "CB")
TENSORDEF_MC(QUint8Crouton4x1_TCM, Crouton4x1_8, DType::QUInt8,
             MemoryClass::TCM, "C#B")
TENSORDEF_MC(QUint8Crouton2x2_TCM, Crouton2x2_8, DType::QUInt8,
             MemoryClass::TCM, "C#B")
TENSORDEF_MC(QUint8WideCrouton_TCM, WideCrouton_8, DType::QUInt8,
             MemoryClass::TCM, "WB")
TENSORDEF_MC(QUint8WideCrouton2x2_TCM, WideCrouton2x2_8, DType::QUInt8,
             MemoryClass::TCM, "W#B")
...
TENSORDEF_MC(QuantUint16_5D_TCM, Flat5D_16, DType::QUInt16, MemoryClass::TCM,
             "F5H")
TENSORDEF_MC(QuantInt16_TCM, Flat_16, DType::QInt16, MemoryClass::TCM, "Fh")
TENSORDEF_MC(QUint16Crouton_TCM, Crouton_16, DType::QUInt16, MemoryClass::TCM,
             "CH")
TENSORDEF_MC(QInt16Crouton_TCM, Crouton_16, DType::QInt16, MemoryClass::TCM,
             "Ch")
...
TENSORDEF_MC(PlainFloat16_5D_TCM, Flat5D_16, DType::Float16, MemoryClass::TCM,
             "F5e")
...
TENSORDEF_MC(QInt32Crouton_TCM, Crouton_32, DType::QInt32, MemoryClass::TCM,
             "Cs")
TENSORDEF_MC(QInt32WideCrouton_TCM, WideCrouton_32, DType::QInt32,
             MemoryClass::TCM, "Ws")
...
TENSORDEF_MC(QFloatCrouton_TCM, Crouton_32, DType::Int32, MemoryClass::TCM,
             "Ct")
TENSORDEF_MC(FloatCrouton_TCM, Crouton_32, DType::Float32, MemoryClass::TCM,
             "Cf")
...
template <> constexpr const char *type_name<const Graph &>()
{
    return "";
}
...
typedef ConcreteTensor<Tdefs::QUint8WideCrouton_TCM>
        QUint8WideCroutonTensor_TCM;
typedef ConcreteTensor<Tdefs::QUint8WideCrouton2x2> QUint8WideCrouton2x2Tensor;
typedef ConcreteTensor<Tdefs::QUint8WideCrouton2x2_TCM>
        QUint8WideCrouton2x2Tensor_TCM;
typedef ConcreteTensor<Tdefs::QInt32WideCrouton> QInt32WideCroutonTensor;
typedef ConcreteTensor<Tdefs::QInt32WideCrouton_TCM>
        QInt32WideCroutonTensor_TCM;
...
typedef LayoutTensor<Ldefs::Crouton_16>
        LayoutCrouton_16; // [1,8,4,32] interleaved
...
using TypicalTensors = std::tuple<
        PlainFloatTensor, PlainFloatTensor5D, PlainFloat16Tensor,
        QuantUint8Tensor, QuantUint8Tensor5D, QuantInt8Tensor, QuantInt8Tensor5D,
        QuantUint16Tensor, QuantUint16Tensor5D, QuantInt16Tensor,
        QuantInt32Tensor, Int32Tensor, Int32Tensor5D, QUint8CroutonTensor,
        QInt8CroutonTensor, QUint8Crouton4x1Tensor, QUint8Crouton2x2Tensor,
        QUint16CroutonTensor, QInt16CroutonTensor, QInt32CroutonTensor,
        QFloatTensor, QFloatCroutonTensor, Int32CroutonTensor,
        PlainFloat16Tensor_TCM, PlainFloat16Tensor5D>;

namespace hnnx {
// these tensor types are 'pre-registered' for deserialize

using CoreTensors = std::tuple<
        PlainFloatTensor, PlainFloatTensor5D, PlainFloat16Tensor, Int32Tensor,
        Int32Tensor5D, PlainFloatTensor_TCM, PlainFloatTensor5D_TCM, Int32Tensor_TCM, QuantUint8Tensor,
        QuantUint8Tensor5D, QuantInt8Tensor, QuantInt8Tensor5D, QuantUint8Tensor_TCM,
        QuantUint8Tensor5D_TCM, QuantInt8Tensor_TCM, QuantInt8Tensor5D_TCM, QuantUint16Tensor,
        QuantUint16Tensor5D, QuantInt16Tensor, QuantUint16Tensor_TCM,
        QuantUint16Tensor5D_TCM, QuantInt16Tensor_TCM, QuantInt32Tensor,
        QUint8CroutonTensor, QuantInt32Tensor_TCM, QUint8CroutonTensor_TCM,
        QInt8CroutonTensor, QUint16CroutonTensor, QInt8CroutonTensor_TCM,
        QUint16CroutonTensor_TCM, QInt32CroutonTensor, QInt16CroutonTensor,
        QInt16CroutonTensor_TCM, QInt32CroutonTensor_TCM,
        QInt32WideCroutonTensor, QInt32WideCroutonTensor_TCM, QFloatTensor,
        QFloatCroutonTensor, Int32CroutonTensor, Int32CroutonTensor_TCM,
        D32FloatTensor, D32PaddedFloatTensor, F16CroutonTensor,
        F16CroutonTensor_TCM, QUint8WideCroutonTensor,
        QUint8WideCroutonTensor_TCM, QUint8Crouton2x2Tensor_TCM,
        QUint8WideCrouton2x2Tensor_TCM, PlainFloat16Tensor_TCM,
...
struct tensor_generator_table_entry {
    ...
    typedef std::unique_ptr<T> (*maketens_funcp)(Op const *, OutputDef const &,
                                                 Graph &);
    ...
    inline constexpr tensor_generator_table_entry(TX const *)
        : rank(tensor_traits<TX>::rank), dtype(tensor_traits<TX>::dtype),
...
inline constexpr std::array<tensor_generator_table_entry,
                            std::tuple_size_v<TTUPLE>>
        make_tengen_init(std::index_sequence<I...>)
{
    return {tensor_generator_table_entry(
            static_cast<typename std::tuple_element_t<I, TTUPLE> *>(
                    nullptr))...};
}
...
template <typename TensorType> struct tensor_generator_lookup {
    ...
    using has_TensorType_as_base =
            std::integral_constant<bool,
                                   std::is_base_of<TensorType, TX>::value>;
    // this is a tuple of types for which T is a common base.
    using applicable_types =
            TupFilter_t<has_TensorType_as_base, TypicalTensors>;
    ...
    static tensor_generator_table_entry const *lookup(int rank, DType dtype)
    {
        ...
        static constexpr std::array<tensor_generator_table_entry, NTYPES>
                typedescs = make_tengen_init<applicable_types>(
                        std::make_index_sequence<NTYPES>{});
        tensor_generator_table_entry const *p = typedescs.data();

        for (int i = 0; i < int(NTYPES); i++) {
            if (p->dtype == dtype && p->rank == rank)
                return p;
            p++;
        }
        return nullptr;
    ...
    static std::unique_ptr<Tensor> make
            [[gnu::noinline]] (const Op *producer_in, const OutputDef &def,
                               Graph &graph_in)
    {
        ...
        if constexpr (!std::is_abstract<TensorType>::value) {
            if (def.dtype == tensor_traits<TensorType>::dtype &&
                ...
                return make_tensor_template<Tensor, TensorType>(producer_in,
                                                                def, graph_in);
            }
        }
        tensor_generator_table_entry const *lookup_result =
                lookup(def.rank, def.dtype);
        if (lookup_result != nullptr) {
            return lookup_result->fp(producer_in, def, graph_in);
        }
        errlog("Lookup in %d tensor types failed (%p: <<%s>>)", int(NTYPES),
               producer_in, get_op_true_name(producer_in));
        return nullptr;
    }
...
constexpr std::unique_ptr<Tensor> (*tensor_generator)(
        const Op *producer_in, const OutputDef &def,
        Graph &graph_in) = tensor_generator_lookup<TensorType>::make;
template <typename TensorType>
inline bool tensor_generator_valid(const Op *producer_in, const OutputDef &def,
                                   Graph &graph_in)
{
...
std::unique_ptr<Tensor> tensor_generator_scalar(const Op *producer_in,
                                                const OutputDef &def,
...
extern GraphStatus tensor_compare(const Tensor &lhs, const Tensor &rhs,
                                  cmp_function fn);
...
struct has_memclass<C, std::tuple<T, Ts...>> {
    static constexpr bool value = memclass_of<T>::memclass == C ||
                                  has_memclass<C, std::tuple<Ts...>>::value;
};
...
template <typename... T> struct reg_tens_for_deser {
    ...
    static inline auto f_ptr() -> int (*)() { return &f; }
};
// empty list
// TODO: make fptr return null (and make the other end ok with it)
...
template <typename T> struct reg_tens_for_deser<T> {
    static int f()
    {
        if constexpr (!(std::is_abstract<T>::value)) {
            deserialize_tensor_register(
                    typeid(T), type_name<T>(),
                    deserialize_tensor_using_constructor<T>::deserialize);
        }
        return 0;
    }
    static inline auto f_ptr() -> int (*)()
    {
        if constexpr (!(std::is_abstract<T>::value)) {
            return &f;
        } else {
...
template <typename TUP, bool FORCE = false> struct deserialize_tensor_tuple {
    template <typename T>
    using not_core_tensor =
            not_contains_type<typename SkipRegTensors<FORCE>::type, T>;
    using filtered_T =
            std::conditional_t<FORCE, TUP,
                               typename TupFilter<not_core_tensor, TUP>::type>;
    using rtfd_type = typename map_rtfd_type<filtered_T>::type;
    static inline auto f_ptr() -> int (*)() { return rtfd_type::f_ptr(); }

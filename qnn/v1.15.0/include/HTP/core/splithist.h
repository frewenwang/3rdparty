...
class splithist_t {
  ...
  public:
    ...
    inline bool operator==(splithist_t const &rhs) const
    {
        return val == rhs.val;
    }
    inline bool operator!=(splithist_t const &rhs) const
...
class SplitHistory {
  ...
  private:
    SplitHistory(unsigned orig_id_in, unsigned unique_id_in,
                 indices_list_t &&spl_in)
        : orig_id(orig_id_in), unique_id(unique_id_in),
          split_indices(std::move(spl_in))
    {
    }
};
...
class SplitHistoryTable {
  ...
  public:
    ...
    splithist_t make_new_split(int dimno, OpId nodeid, splithist_t oldhist,
                               int nslices, int slice_size);
    ...
    {
        return ((a == b) || b.empty()) ? a
               : a.empty()             ? b
                                       : resolve_func(a, b, is_const);
    }
  ...
  protected:
    ...
    struct cursor {
        unsigned
                rec_index; // current record's index (skipping indirect record if any)
    ...
    void cursor_to_parent(
            cursor &c) const; // move to parent, only legal if not root.
    unsigned
    extract_orig_id(unsigned recindex) const; // only to be used on root record.
};

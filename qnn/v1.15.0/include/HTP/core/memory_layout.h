...
static inline constexpr std::array<T, Rank>
make_stdarray_helper(const T val, std::index_sequence<I...>)
{
...
template <size_t RankVal> struct ChunkedMemoryLayout<RankVal> {
    ...
    static constexpr std::array<size_t, Rank> ChunkSizes =
            hnnx::make_stdarray<size_t, Rank>(1);
    static constexpr size_t chunk_total = 1;
    static constexpr bool is_valid_chunk = true;
    static inline constexpr size_t
    chunk_offset(const std::array<size_t, Rank> &padded_coords,
                 const std::array<size_t, Rank> &dims_total)
    {
        return 0;
    }
    static inline constexpr size_t
    linear_offset(const std::array<size_t, Rank> &padded_coords,
                  const std::array<size_t, Rank> &dims_total)
    {
        return 0;
    }
    ...
    static inline constexpr std::array<size_t, Rank>
    pad_dims(const std::array<size_t, Rank> dims_in)
    {
        return dims_in;
    }
};
...
struct ChunkedMemoryLayout<RankVal, Dim, ChunkSize, Rest...> {
    ...
    static constexpr bool is_valid_chunk =
            ((ChunkSize > 0) && (Smaller::is_valid_chunk));
    static constexpr std::array<size_t, Rank>
    embiggen_chunksize(const std::array<size_t, Rank> smaller_chunksize)
    {
        std::array<size_t, Rank> out = smaller_chunksize;
        if (ChunkSize)
            std::get<Dim>(out) *= ChunkSize;
        return out;
    }
    static constexpr std::array<size_t, Rank> ChunkSizes =
            embiggen_chunksize(Smaller::ChunkSizes);
                 ...
                 const std::array<size_t, Rank> &dims_total)
    {
        if constexpr (ChunkSize > 0) {
            const size_t smaller_offset =
                    Smaller::chunk_offset(padded_coords, dims_total);
            const size_t dim_coord = padded_coords[Dim];
            const size_t smaller_idx =
                    dim_coord / std::get<Dim>(Smaller::ChunkSizes);
            ...
            size_t chunk_off = Smaller::chunk_offset(padded_coords, dims_total);
            return chunk_off;
        }
    }
                ...
                const std::array<size_t, Rank> &dims_total, size_t offset = 0)
    {
        if constexpr (is_valid_chunk) {
            ...
            size_t chunk_idx =
                    Smaller::chunk_index(padded_coords, dims_total, offset);
            return chunk_idx;
        }
    }
    static inline constexpr size_t
    ...
    static inline std::array<size_t, Rank>
    pad(const std::array<size_t, Rank> dims_in)
    {
        std::array<size_t, Rank> newdims;
        for (int i = 0; i < Rank; i++) {
            auto dim_chunk_size = ChunkSizes[i];
            newdims[i] = ((dims_in[i] + (dim_chunk_size - 1)) &
                          (~(dim_chunk_size - 1)));
        }
        return newdims;
    }
...
template <size_t RankVal> struct FlatMemoryLayout {
    ...
    static inline constexpr size_t
    num_blocks(const std::array<size_t, Rank> max_dims)
    {
...
class R4NCHWMemoryLayout
    : public ChunkedMemoryLayout<4, 0, 0, 3, 0, 2, 0, 1, 0> {
}; // NCHW
class R4Depth32MemoryLayout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 3, 0, 2, 0, 2, 4, 3, 32> {
};

// Croutons for HMX, YYYXXXDDDDD chunks
class R4CroutonLayout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 8, 2, 8, 3, 32> {
};
// Croutons for HMX, YXXXXXDDDDD chunks (wide aspect ratio)
class R4WideCroutonLayout : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3,
                                                       0, 1, 2, 2, 32, 3, 32> {
};

// Croutons for HMX, YYYXDDDDDXX chunks
class R4Crouton4x1Layout : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0,
                                                      1, 8, 2, 2, 3, 32, 2, 4> {
};

// Croutons for HMX, YYXXDDDDDYX chunks
class R4Crouton2x2Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 4, 2, 4, 3, 32,
                                 1, 2, 2, 2> {
};

// Croutons for HMX, YYXXDDDDDYX chunks (wide aspect ratio)
class R4WideCrouton2x2Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 2, 16, 3, 32, 1, 2,
                                 2, 2> {
};

// Croutons2 for HMX, 8x4x32 chunks where the data is 16b
class R4Crouton2Layout : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0,
                                                    1, 8, 2, 2, 3, 32, 2, 2> {
};

class R4Crouton4Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 8, 2, 2, 3, 32> {
};
...
class R4Weights8x4Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 3, 0, 2, 0, 0, 8, 1, 4, 2, 16,
                                 3, 32, 2, 2> {
};

class R5CroutonLayout : public ChunkedMemoryLayout<5, 0, 0, 1, 0, 2, 0, 3, 0, 4,
                                                   0, 2, 8, 3, 8, 4, 32> {
};

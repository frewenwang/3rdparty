...
{
    return Q6_Vw_vmpyoacc_VwVwVh_s1_rnd_sat_shift(Q6_Vw_vmpye_VwVuh(vu, vv), vu,
                                                  vv);
}

inline HVX_Vector q6op_Vw_vmpy_VwVw_s1_sat(HVX_Vector vu, HVX_Vector vv)
{
    return Q6_Vw_vmpyoacc_VwVwVh_s1_sat_shift(Q6_Vw_vmpye_VwVuh(vu, vv), vu,
                                              vv);
}
...
{
    unsigned int bL = (uintptr_t)addr;
...
l2pref(const void *p, uint32_t height, uint32_t width, uint32_t stride)
{
    uint64_t control =
            Q6_P_combine_RR(stride, Q6_R_combine_RlRl(width, height));
...
inline void dcfetch(void const *addr) {}
inline void l2pref(const void *p, uint32_t height, uint32_t width,
                   uint32_t stride)
{
}
...
void hvx_store_vec_x2_unaligned(void *addr, HVX_Vector v0, HVX_Vector v1,
                                int bytes) noexcept;

inline void hvx_store_vec_x2_unaligned_inline(void *addr, HVX_Vector v0,
                                              HVX_Vector v1, int bytes) noexcept
{
    check_hvx();

    unsigned int VECTOR_SIZE = 128;
...
void hvx_store_vec_x4_unaligned(void *addr, HVX_Vector v0, HVX_Vector v1,
                                HVX_Vector v2, HVX_Vector v3,
                                int bytes) noexcept;

inline void hvx_store_vec_x4_unaligned_inline(void *addr, HVX_Vector v0,
                                              HVX_Vector v1, HVX_Vector v2,
...
inline HVX_Vector uint64_to_qfloat(HVX_Vector ll_hi, HVX_Vector ll_lo)
{
    ...
    exp = Q6_Vw_vsub_VwVw(
            v32, exp0); //compute missing bit using oppisite shift on lo
...
inline HVX_Vector uint64_to_float(HVX_VectorPair bigval)
{
    return Q6_Vsf_equals_Vqf32(
            uint64_to_qfloat(Q6_V_hi_W(bigval), Q6_V_lo_W(bigval)));
}
...
static inline __attribute__((unused, always_inline)) HVX_Vector
Q6_Vqf32_equals_Vsf(HVX_Vector vin)
{
    HVX_Vector vzero = Q6_V_vsplat_R(0);
...
static inline __attribute__((unused, always_inline)) HVX_Vector
Q6_Vqf32_from_int(HVX_Vector vin)
{
...
inline void __attribute__((always_inline))
scatter_release_and_stall(const void *p) // must point to TCM
{
...
scatter_release_and_stall(const void *p)
{
    check_hvx();

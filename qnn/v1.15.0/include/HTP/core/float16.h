...
struct Float16 {
  ...
  private:
    ...
    uint16_t sign_bit() const;
    uint16_t exp_bits() const;
    uint16_t frac_bits() const;
...
inline constexpr Float16::Float16(float f) : d(0)
{
    ...
    bool neg = u.w & (1u << 31);
    int exp_extract = (u.w >> 23) & 0xFF;
    uint32_t frac_bits = u.w & 0x7FFFFF;
    ...
    int exp = exp_extract - 127;
    int frac = round(frac_bits | (1u << 23), 23 - 10);
    d = make(neg, exp, frac);
}
...
inline constexpr bool Float16::is_norm() const
{
    if (is_zero())
        return true;
    return exp_bits() > make_exp_bits(0x0000) &&
           exp_bits() < make_exp_bits(0x001F);
}
...
{
    assert(is_finite());
    int16_t e = static_cast<int16_t>(exp_bits() >> 10);
...
inline constexpr Float16::operator float() const
{
    ...
    float v = ldexpf(f, e - 10);
    return sign ? -v : v;
}

inline constexpr Float16::Float16(int sign, int exp, int frac)
    : d(make(sign, exp, frac))
{
}

inline uint16_t Float16::sign_bit() const
{
    return d & 0x8000;
}

inline uint16_t Float16::exp_bits() const
{
    return d & 0x7C00;
}

inline uint16_t Float16::frac_bits() const
{
    return d & 0x03FF;
}
...
{
    uint16_t f = quiet ? 0x0200 : 0x0100;
...
inline constexpr uint16_t Float16::make(int sign, int exp, int frac)
{
    ...
    assert(frac > 0);
    unsigned clz = __builtin_clz(frac);
    ...
    if (exp_inc < 0) {
        ...
        unsigned clzr = __builtin_clz(frac);
...
inline std::pair<int32_t, int32_t> Float16::force_norm() const
{
    ...
    if (e == 0) {
        // Subnormal number.
        assert(f != 0);
        unsigned clz = __builtin_clz(f) - 16; // Pretend we have 16 bits.

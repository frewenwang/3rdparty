...
template <class HASHFUNC> class string_registry_two {
  ...
  protected:
    ...
    char *need_bulk(size_t n)
    {
        ...
        char *res = m_bulk_current + m_bulk_pos;
        m_bulk_pos += n;
        return res;
    }
...
template <class HASHFUNC>
hnnx::string_registry_two<HASHFUNC>::string_registry_two()
...
hnnx::string_registry_two<HASHFUNC>::map_str(char const *s)
{
    return map_str(std::string_view(s));
}
template <class HASHFUNC>
typename string_registry_two<HASHFUNC>::string_key
hnnx::string_registry_two<HASHFUNC>::map_str(std::string const &s)
{
    return map_str(std::string_view(s));
}
...
template <class HASHFUNC>
typename string_registry_two<HASHFUNC>::string_key
hnnx::string_registry_two<HASHFUNC>::map_str(std::string_view s)
{
    size_t slen = s.size();
    if (slen == 0)
        return &empty_string_node; // empty string
    ...
    if (found != m_fwd_map.end() && s == found->first)
        return &*found;

    // ok, now we have to do an insert. first put the string in bulk storage
    //
    char *dst = need_bulk(slen + 1);
    memcpy(dst, s.data(), slen);
    dst[slen] = '\0';

    unsigned hash = get_hash(s);

    auto ins_iter = m_fwd_map.emplace_hint(
            found, std::make_pair(std::string_view(dst, slen), hash));
    return &*ins_iter;
}
template <class HASHFUNC>
typename string_registry_two<HASHFUNC>::string_key

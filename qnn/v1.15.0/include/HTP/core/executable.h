...
class Executable { // Almost certainly, don't make this a parent class!
  public:
    using FuncType = GraphStatus (*)(const void *);
    using ItemType = std::pair<FuncType, const void *>;
    using ExecType = std::tuple<FuncType, const void *, counter_t, counter_t>;
    virtual GraphStatus
    execute() const noexcept = 0; // Needs to be at vtable offset zero!!!
    virtual ItemType compile()
            const; // Turn this Executable into a function pointer and data pointer.
    virtual ~Executable() = default;
    static const size_t *
    vtable(Executable const *); // helper function: get vtable
    static const size_t execute_address(
            Executable const
                    *); // helper function: get address of execute() function

    static GraphStatus no_op_function(const void *); // just returns Success.
    static ItemType null_item() { return {no_op_function, nullptr}; }
};

// to execute an Executable::ItemType...

inline GraphStatus execute_item(Executable::ExecType const &itemt)
{
    return (*std::get<0>(itemt))(std::get<1>(itemt));
}

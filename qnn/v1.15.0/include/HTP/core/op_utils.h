...
template <typename T>
static inline bool is_output_def_valid(const OutputDef &output_def,
                                       Graph &graph_in)
{
...
template <typename T>
static inline bool is_input_tensor_compatible(Graph &graph_in,
                                              Tensor const *tensor)
{
    ...
    if constexpr (0)
        debuglog("input tensor is %p dynamic cast to %s is %p", tensor,
                 typeid(T).name(), dynamic_cast<T>(tensor));
...
static inline bool
are_output_defs_valid_helper(std::index_sequence<I...>,
                             ...
                             Graph &graph_in)
{
    ...
    return (is_output_def_valid<std::tuple_element_t<I, TupType>>(
                    *outputs_in[I], graph_in) &&
            ...);
}

template <size_t N, typename TupType>
static inline bool are_output_defs_valid(OutputDef const *const *outputs_in,
                                         Graph &graph_in)
{
    return are_output_defs_valid_helper<TupType>(std::make_index_sequence<N>{},
                                                 outputs_in, graph_in);
}

template <typename TupType, size_t... I>
static inline bool
are_input_tensors_compatible_helper(std::index_sequence<I...>, Graph &graph_in,
                                    Tensor const *const *inputs_in)
{
    return ((is_input_tensor_compatible<std::tuple_element_t<I, TupType>>(
                    graph_in, inputs_in[I])) &&
            ...);
}

template <size_t N, typename TupType>
static inline bool are_input_tensors_compatible(Graph &graph_in,
                                                Tensor const *const *inputs_in)
{
    return are_input_tensors_compatible_helper<TupType>(
            std::make_index_sequence<N>{}, graph_in, inputs_in);
}

template <typename T>
std::unique_ptr<Tensor> tensor_output_alloc(const Op *producer_in,
...
typedef std::unique_ptr<Tensor> (*tensor_generate_fp)(Op const *,
                                                      OutputDef const &,
                                                      Graph &);
//
...
inline constexpr std::array<tensor_generate_fp, N>
        tensor_gen_array_helper(std::index_sequence<I...>)
{
    return {tensor_generator<std::tuple_element_t<I, TupType>>...};
}
template <typename TupType>
inline constexpr std::array<tensor_generate_fp, std::tuple_size_v<TupType>>
tensor_gen_array()
{
...
inline tensor_generate_fp const *tensor_gen_array_ptr()
{
    if constexpr (std::tuple_size_v < TupType >> 0) {
        static constexpr std::array<tensor_generate_fp,
                                    std::tuple_size_v<TupType>>
                ptr_array = tensor_gen_array<TupType>();
...
find_output_size_and_prepad(bool same_shape, size_t insize, size_t winsize,
                            size_t stride)
{
    size_t outsize;
    size_t prepad;

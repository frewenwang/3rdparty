...
template <typename R, typename... Args> class OptFunction<R(Args...)> {
  public:
    ...
    {
        OptFunctionType obj = (OptFunctionType)t;
        return obj(args...);
    }
    template <typename L>
    static typename std::enable_if<!std::is_lvalue_reference_v<L>,
                                   thisType>::type
    create(L &&lambda)
    {
        L *l = get_lambda_crate()->emplace<L>(std::forward<L>(lambda));
...
template <> struct wrapper_helper<float> {
    static constexpr auto wrap(float x)
    {
        return expr<Variant::value, float>(x);
    }
};
template <> struct wrapper_helper<DType> {
    static constexpr auto wrap(DType x)
    {
        return expr<Variant::value, DType>(x);
    }
};

// double->float
template <> struct wrapper_helper<double> {
    static constexpr auto wrap(double x)
    {
        return expr<Variant::value, float>(float(x));
    }
};
...
{
    return wrapper_helper<std::remove_const_t<std::remove_reference_t<T>>>::
            wrap(forward<T>(p));
}
...
template <typename T2, typename T> inline constexpr auto wrap_param_to(T &&p)
{
    ...
    if constexpr (std::is_same_v<bool, T2> && std::is_arithmetic_v<Tparam>) {
        ...
        return wrapper_helper<Tparam>::wrap(forward<T>(p));
    }
}
...
class expr<Variant::binop, tuple<A, B, OPER>> {
  ...
  public:
    ...
    {
        return m_op(m_a.eval(e), m_b.eval(e));
    }
};
template <typename A, typename B, typename OPER>
...
inline constexpr auto make_binop(expr<Variant::value, TA> a,
                                 expr<Variant::value, TB> b, OPER op)
{
...
reduce_logop(ECtx &e, OPERANDS const &operands)
{
    ...
    if constexpr (IDX + 1 >= N) { // it's the last one .. we're done
        ...
        if constexpr (VAR == Variant::lg_and) {
            if (!result_i)
                return false; // short-cut AND if false
        } else if constexpr (VAR == Variant::lg_or) {
            if (result_i)
                return true; // short-cut OR if true
        } else {
...
{
    return expr<WHICH, std::remove_reference_t<OPERANDS>>(
            forward<OPERANDS>(opers));
}
// select
...
class expr<Variant::select, tuple<SEL, A, B>> {
  ...
  public:
    ...
    constexpr otype eval(ECtx &e) const
    {
        return m_sel.eval(e) ? m_a.eval(e) : m_b.eval(e);
    }
};
template <typename SEL, typename A, typename B>
...
#define OEXP_ARITH(FNAME, CLST)                                                \
    ...
    {                                                                          \
        if constexpr (std::is_arithmetic_v<std::remove_reference_t<A>> &&      \
            ...
            /* 'constant folding' ... */                                       \
            using common_t = std::common_type_t<std::remove_reference_t<A>,    \
                                                std::remove_reference_t<B>>;   \
            return CLST<common_t>()(pa, pb);                                   \
        } else {                                                               \
            auto wa = wrap_param(forward<A>(pa));                              \
            auto wb = wrap_param(forward<B>(pb));                              \
...
template <typename T> struct func_roundup {
    static_assert(std::is_integral_v<T>,
                  "ROUNDUP can only apply to integer types");
...
{
    static_assert(std::is_integral_v<T>,
                  "IS_POW2 can only apply to integer types");
    return (a & (a - 1)) == 0;
}
...
{
    auto wa = wrap_param(forward<TA>(a));
...
{
    auto parms = std::make_tuple(wrap_param_to<bool>(forward<TA>(a)),
                                 wrap_param_to<bool>(forward<TB>(b)),
                                 wrap_param_to<bool>(forward<Ts>(ts))...);
...
{
    return ADD(ADD(forward<TA>(a), forward<TB>(b)), forward<Ts>(ts)...);
}
...
{
    return MUL(MUL(forward<TA>(a), forward<TB>(b)), forward<Ts>(ts)...);
}
...
{
    return MIN(MIN(forward<TA>(a), forward<TB>(b)), forward<Ts>(ts)...);
}
//! MAX(a,b,c...) - equivalent to MAX( MAX(a,b), c ...)
template <typename TA, typename TB, typename... Ts>
inline constexpr auto MAX(TA &&a, TB &&b, Ts &&...ts)
{
    return MAX(MAX(forward<TA>(a), forward<TB>(b)), forward<Ts>(ts)...);
}
...
template <typename T, typename EX> sFunction<T> wrap_as_function(EX &&a)
{
    ...
    if constexpr (std::is_same_v<EXT, sFunction<T>>) {
        ...
        auto ftmp = wrap_param_to<T>(forward<EX>(a));
        return sFunction<T>::create(
                [ftmp](ECtx &e) -> T { return ftmp.eval(e); });
    }
}

...
class MatchAstSubnode {
  ...
  protected:
    ...
    operand_tag_t
            m_optag; ///<   m_optag contains the operand tag if the input is not a contained op ; "" otherwise
  public:
...
class MatchAstNode {
  ...
  public:
    ...
    operand_tag_t
            m_optag; ///< tag assigned via LET, or empty if none ("*" at root)
  ...
  protected:
    ...
    std::vector<MatchAstSubnode>
            m_subnodes; ///< One for each input in the pattern

    /// Count the ops, and put their LET names in the map.
    /// Returns >1 if ok, -1 if error
    int enumerate_ops(std::map<operand_tag_t, int> &op_tag_to_idx_map,
                      int opcount);

    // if WITH_OPT_DEBUG is not defined, this returns an empty pointer.
    std::unique_ptr<char[]> make_debug_desc(
            std::map<operand_tag_t, int> const &opertag_to_idx_map) const;

  public:
    MatchAstNode(char const *, char const *, node_variant, int n_subnodes,
...
class MatchBuilder {
  ...
  protected:
    ...
    template <typename... S>
    using have_matchast_uptr =
            std::disjunction<std::is_same<hnnx::MatchAst_uptr, S>...>;

    // just convert each 'input' to a MatchAstSubnode, and build a MatchAstNode from that.
    template <typename... Ts>
    static typename std::enable_if<have_matchast_uptr<Ts...>::value,
                                   hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden")))
            Op(char const *opname, Ts &&...ts)
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts)] = {
                std::move(std::forward<Ts>(ts))...};
        return std::make_unique<hnnx::MatchAstNode>(opname, pkg_flag.c_str(),
                                                    hnnx::MatchAstNode::is_Op,
                                                    sizeof...(Ts), subnodes);
    }

    template <typename... Ts>
    static typename std::enable_if<are_strings<Ts...>::value,
                                   hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden")))
            Op(char const *opname, Ts... ts)
    {
    ...
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts) + 1] = {std::move(t1),
                                                             (ts)...};
        return std::make_unique<hnnx::MatchAstNode>(
                opname, pkg_flag.c_str(), hnnx::MatchAstNode::is_Op,
            ...
            OpVarIn(char const *opname, Ts &&...ts)
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts)] = {
                std::move(std::forward<Ts>(ts))...};
        return std::make_unique<hnnx::MatchAstNode>(
                opname, pkg_flag.c_str(), hnnx::MatchAstNode::is_OpVarIn,
                sizeof...(Ts), subnodes);
    }
    ...
    //
    static void implement_LET(hnnx::operand_tag_parm_t optag,

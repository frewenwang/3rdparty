...
template <typename TI> struct intrange_within_float<Float16, TI> {
    ...
    static inline constexpr Float16 max()
    {
        if constexpr (sizeof(TI) < 2) {
            ...
            return std::numeric_limits<TI>::is_signed ? 32752.0_f16
                                                      : 65504.0_f16;
        } else {
            return std::numeric_limits<TI>::is_signed ? -65504.0_f16
                                                      : 65504.0_f16;
        }
    }
    // 'min' value of integer range is always exactly representable
    static inline constexpr Float16 min()
    {
        return Float16(std::numeric_limits<TI>::min());
    }
};
...
template <typename TI> struct intrange_within_float<float, TI> {
    ...
    static inline constexpr float max()
    {
        if constexpr (sizeof(TI) < 4) {
            ...
            return std::numeric_limits<TI>::is_signed ? 2147483520.0f
                                                      : 4294967040.0f;
        } else {
            static_assert(sizeof(TI) == 8);
            return std::numeric_limits<TI>::is_signed ? 9.223371487e18f
                                                      : 1.844674297e+19f;
        }
    }
    // 'min' value of integer range is always exactly representable
    static inline constexpr float min()
    {
        return float(std::numeric_limits<TI>::min());
    }
};
...
template <typename TI> struct intrange_within_float<double, TI> {
    ...
    static inline constexpr double max()
    {
        if constexpr (sizeof(TI) < 8) {
            ...
            static_assert(sizeof(TI) == 8);
            return std::numeric_limits<TI>::is_signed ? 9223372036854774784.0
    ...
    static inline constexpr float min()
    {
        return double(std::numeric_limits<TI>::min());
    }
};
...
template <typename TOUT, typename TIN> struct satcast_helper {
    ...
    static inline TOUT constexpr op(TIN val)
    {
        if constexpr (!std::numeric_limits<
                              TOUT>::is_integer) { // convert to a float
            return TOUT(val);
        } else {
            ...
            if constexpr (std::numeric_limits<TIN>::is_integer) {
                ...
                if (sizeof(TOUT) > sizeof(TIN) ||
                    (sizeof(TOUT) == sizeof(TIN) && OUTS == INS)) {
                    ...
                    if (!OUTS) {
                        ...
                        constexpr TIN mn =
                                INS ? std::numeric_limits<TOUT>::min() : 0;
                        constexpr TIN mx = std::numeric_limits<TOUT>::max();
                        return (val < mn)   ? TOUT(mn)
                               : (val > mx) ? TOUT(mx)
                                            : TOUT(val);
                    }
                }
                ...
                if constexpr (sizeof(TOUT) <= sizeof(int32_t)) {
                    if constexpr (OUTS) {
                        constexpr TIN loval =
                                intrange_within_float<TIN, int32_t>::min();
                        constexpr TIN hival =
                                intrange_within_float<TIN, int32_t>::max();
                        int32_t tmp =
                                (int32_t)std::max(loval, std::min(hival, val));
                        return satcast_helper<TOUT, int32_t>::op(tmp);
                    } else {
                        constexpr TIN loval = 0.0;
                        constexpr TIN hival =
                                intrange_within_float<TIN, uint32_t>::max();
                        uint32_t tmp =
                                (uint32_t)std::max(loval, std::min(hival, val));
                        return satcast_helper<TOUT, uint32_t>::op(tmp);
                    }
                } else { // 64-bit output assumed
                    constexpr TIN loval =
                            intrange_within_float<TIN, TOUT>::min();
                    constexpr TIN hival =
                            intrange_within_float<TIN, TOUT>::max();
...
template <typename TOUT> inline TOUT saturate_round(float val)
{
    ...
    if constexpr (sizeof(TOUT) == 4 && !std::numeric_limits<TOUT>::is_signed) {
        ...
        int r = Q6_R_convert_sf2w_R(val);
        if constexpr (sizeof(TOUT) < 4)
            return hnnx::scast::q6_sat_int<TOUT>::op(r);
        return r;
    }
}
#endif
...
template <typename TA, typename TB> struct proper_cmp_helper {
    static_assert(std::numeric_limits<TA>::is_integer &&
    ...
    static const bool CMP_AS_INT = (sizeof(TA) < sizeof(int) ||
                                    (sizeof(TA) == sizeof(int) && ASIGNED)) &&
                                   (sizeof(TB) < sizeof(int) ||
                                    (sizeof(TB) == sizeof(int) && BSIGNED));
    // otherwise, compare by promoting B to A when ...
    static const bool B_TO_A = sizeof(TA) > sizeof(TB) ||
                               (sizeof(TA) == sizeof(TB) && !ASIGNED);
    ...
    static inline bool constexpr eq(TA a, TB b)
    {
        if (CMP_AS_INT) {
            ...
            if (!ASIGNED && BSIGNED && b < 0)
                return false;
            return a == (TA)b;
        } else {
            if (!BSIGNED && ASIGNED && a < 0)
                return false;
            return (TB)a == b;
        }
    }
    static inline bool constexpr lt(TA a, TB b)
    {
        if (CMP_AS_INT) {
            ...
            if (!ASIGNED && BSIGNED && b < 0)
                return false; // a < b  always false if  b<0
            return a < (TA)b;
        } else {
            if (!BSIGNED && ASIGNED && a < 0)
                return true; // a < b  always true if  a<0
            return (TB)a < b;
        }
    }
};
...
inline int64_t /*constexpr*/ muli64_sat(int64_t a, int64_t b)
{
    int64_t prod = 0;
    if (__builtin_mul_overflow(a, b, &prod)) {
        prod = ((a ^ b) >= 0) ? std::numeric_limits<int64_t>::max()
...
{
    static_assert(std::is_integral_v<TD>,
                  "round_up can only apply to integer types");

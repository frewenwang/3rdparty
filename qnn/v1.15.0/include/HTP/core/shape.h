...
shape_reduce_map::iterator shape_find_in_map(shape_reduce_map &map,
                                             void const *shp, unsigned hash,
                                             unsigned shplen);

void shape_serialize(Serializer &sctx, unsigned rank, size_t const *dims,
                     size_t const *max_dims, uint8_t const *pad);
...
struct ShapeFlags {
  ...
  public:
    ...
    inline bool is_const_memory() const
    {
        return (flags & unsigned(ShapeFlag::constant)) != 0;
    }
    inline bool is_uncached_memory() const
    {
        return (flags & unsigned(ShapeFlag::uncached)) != 0;
    }
    inline bool ok_src_bypass() const
    {
        return (flags & (unsigned(ShapeFlag::uncached) |
                         unsigned(ShapeFlag::uncached))) != 0;
    }
    inline bool ok_dst_bypass() const
    {
        return (flags & unsigned(ShapeFlag::uncached)) != 0;
    }
...
ShapeFlags const *copy_shape_with_flags(Graph &gr, ShapeFlags const *ref_shape,
                                        unsigned rank, ShapeFlag newflags);
...
template <size_t Rank> struct Shape : public hnnx::ShapeFlags {
    ...
    explicit Shape(const size_t *dims_in)
        : dims(hnnx::ptr_to_stdarray<Rank, size_t>(dims_in)),
          max_dims(hnnx::ptr_to_stdarray<Rank, size_t>(dims_in)), pad(){};
    Shape(std::array<size_t, Rank> dims_in,
          std::array<size_t, Rank> max_dims_in)
        : dims(dims_in), max_dims(max_dims_in), pad(){};
    //  copy, but change the flags
    Shape(Shape const &ref, hnnx::ShapeFlag newflags) : Shape(ref)
    {
    ...
    {
        return std::memcmp(this, &rhs, shplen()) < 0;
    }
    static const Shape *deserialize(hnnx::Deserializer &dctx);
    void serialize(hnnx::Serializer &sctx) const;

  protected:
    unsigned shplen() const
    {
        return (char const *)&pad[0] + Rank - (char const *)this;
    }
};
...
using Shapes = hnnx::shape_reduce_map[6];

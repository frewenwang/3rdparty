 ...
 *
 *    - For qint32 data (and for int32,float), the default tile is 2x8x32; in order to
 ...
 * Note that specifying a height >2 for 32-bit data means you need a buffer larger than 2K (up to 8K).
 *
 *
...
uint8_t const *unsupported_read(Tensor const *, unsigned flags_unused,
                                uint8_t *buf);
...
template <unsigned FLAGS, typename T> struct tile_support_flags_for {
    static constexpr unsigned value =
            FLAGS | ((sizeof(T) == 1) ? Tensor::tile_8bit : 0) |
...
template <typename Linfo> struct tile_methods {
    ...
    static constexpr bool is_generic =
            Rank == 4 && (std::is_same_v<storage_type, uint8_t> ||
                          std::is_same_v<storage_type, uint16_t> ||
    ...
    tile_read(LayoutTensorType const *tensor, // tensor to read from
              unsigned flags,
              uint8_t *tbuf, // caller-supplied buffer
              size_t b, int h, int w, int d) // coordinates
    {
        if constexpr (is_generic) {
            return tileExt_priv::generic_tile_read<storage_type, Rank>(
                    tensor, flags, tbuf, b, h, w, d);
        } else {
    ...
    static inline void
    tile_write(LayoutTensorType *tensor, // tensor to write to
               unsigned flags,
               uint8_t const *tbuf, // caller-supplied buffer
               size_t b, int h, int w, int d)
    {
        if constexpr (is_generic) {
            tileExt_priv::generic_tile_write<storage_type, Rank>(
                    tensor, flags, tbuf, b, h, w, d);
        } else {
...
template <typename Linfo> struct tile_methods_r4flat {
    ...
    tile_read(TensType const *tensor, // tensor to read from
              unsigned flags,
              uint8_t *tbuf, // caller-supplied buffer
              size_t b, int h, int w, int d);
...
template <>
struct tile_methods<Ldefs::Flat_16>
    : public tile_methods_r4flat<Ldefs::Flat_16> {
};
template <>
struct tile_methods<Ldefs::Flat_32>
    : public tile_methods_r4flat<Ldefs::Flat_32> {
};
...
template <typename Linfo> struct tile_methods_r4crouton {
    ...
    {
        using storage_type = typename Linfo::storage_type;
        // 32-bit croutons can't be direct mapped.
        constexpr unsigned direct =
                (sizeof(storage_type) != 4) ? Tensor::tile_direct : 0;
        return tile_support_flags_for<Tensor::tile_fast | direct,
                                      storage_type>::value;
    }
};
...
// 8 bit
template <>
struct tile_methods<Ldefs::Crouton_8>
    : public tile_methods_r4crouton<Ldefs::Crouton_8> {
};
// 16 bit (different layout!)
template <>
struct tile_methods<Ldefs::Crouton_16>
    : public tile_methods_r4crouton<Ldefs::Crouton_16> {
};

// 32 bit
template <>
struct tile_methods<Ldefs::Crouton_32>
    : public tile_methods_r4crouton<Ldefs::Crouton_32> {
};
...
inline void *Tensor::write_tile_strategy(unsigned flags, void *buffer, size_t b,
                                         int h, int w, int d)
{
    unsigned newflags =
            (flags & tileExt::write_strategy_keep) | tileExt::write_strategy;
    void const *res =
            const_cast<Tensor &>(*this).read_tile(newflags, buffer, b, h, w, d);
...
void const *LayoutTensor<Linfo>::read_tile(unsigned flags, void *buffer,
                                           size_t b, int h, int w, int d) const
{
    return (void const *)hnnx::tileExt_priv::tile_methods<Linfo>::tile_read(
            this, flags, (uint8_t *)buffer, b, h, w, d);
}
template <typename Linfo>
void LayoutTensor<Linfo>::write_tile(unsigned flags, void const *buffer,
                                     size_t b, int h, int w, int d)
{
    hnnx::tileExt_priv::tile_methods<Linfo>::tile_write(
            this, flags, (uint8_t const *)buffer, b, h, w, d);
}
template <typename Linfo>
unsigned LayoutTensor<Linfo>::tile_support_bits() const
{
...
template <typename TENST> class tile_support_test {
    ...
    using methods = hnnx::tileExt_priv::tile_methods<
            typename layout_def_of<LTYPE>::type>;
...
class TileStoreWindowBase {
  ...
  public:
    ...
    std::pair<size_t const *, size_t> get_windims() const noexcept
    {
        return {winsize, RANK};
    }
    ...
    TileStoreWindowBase(Tensor &otensor, TensorShape<RANK> const &out_shape,
                        unsigned elbytes)
    {
        ptr = ptrw = otensor.raw_data();
        size_t stride = elbytes;
        for (int i = RANK-1; i >= 0; --i) {
            unsigned dim = out_shape.dim(i);
    ...
    TileStoreWindowBase(Tensor &otensor, ITType const &itens,
    ...
    inline void set_window(TType const &tens, TensorShape<RANK> const &offset)
    {
...
template <unsigned ELBYTES, unsigned RANK = 4> class TileStoreWindow : public TileStoreWindowBase<RANK> {
  public:
    ...
    template <typename ITType>
    TileStoreWindow(Tensor &otensor, ITType const &itens,
                    TensorShape<RANK> const &offset,
    ...
    // is 2 (for ELBYTES=4), otherwise 8.
    //
    // The input 'tiledata' must be a vector aligned pointer to 'tht' tile row.
    // For ELBYTES=1 or 2, a tile row is 256 bytes; for ELBYTES=4, it is 1024.
    ...
    //
    void write_tile(unsigned flags, void const *tiledata, size_t b, int h,
                    int w, int d);
...
template <DType DT> class TileStoreWindowTensor : public FakeTensor {
  ...
  public:
    ...
    TileStoreWindowTensor(Tensor &otensor, TensorShape<4> const &out_shape)
        : FakeTensor(nullptr), ts_window(otensor, out_shape),
          intfc(otensor.interface())
    {
    }

    template <typename ITType>
    TileStoreWindowTensor(Tensor &otensor, ITType const &itens,
                          TensorShape<4> const &offset,
                          TensorShape<4> const &out_shape)
        : FakeTensor(nullptr), ts_window(otensor, itens, offset, out_shape),
          intfc(otensor.interface())
    {
    }

    template <typename TType>
    inline void set_window(TType const &tens, TensorShape<4> const &offset)
    {
  ...
  public:
    ...
    virtual Interface const &interface() const noexcept override final
    {
        return intfc;
    }
    ...
    virtual void const *read_tile(unsigned flags, void *buffer, size_t b, int h,
                                  int w, int d) const override final
    {
    ...
    {
        return hnnx::tileExt_priv::tile_support_flags_for<tile_fast,
                                                          element_type>::value;
    }
};

template <unsigned ELBYTES> class TileStoreWindowTensorGeneric {
    static_assert(false && ELBYTES,
                  "not specialized for this value of ELBYTES");
};

template <>
class TileStoreWindowTensorGeneric<1>
    : public TileStoreWindowTensor<DType::QUInt8> {
};
template <>
class TileStoreWindowTensorGeneric<2>
    : public TileStoreWindowTensor<DType::QUInt16> {
};
template <>
class TileStoreWindowTensorGeneric<4>
    : public TileStoreWindowTensor<DType::Int32> {
};
...
int raw_copy_by_tiles(Tensor &out, Tensor const &in, unsigned flags = 0);
...
//
template <unsigned ELBYTES>
int unary_by_tiles(Tensor &out, Tensor const &in, UnaryTileFunctor const &f,
                   unsigned flags = 0);

...
class OpDefFlags {
  ...
  protected:
    ...
    static inline int flag_init(hnnx::opname_tag_parm_t opstr, int n_in,
                                int n_out)
    {
        std::string_view sv{opstr};
        char c0;
    ...
    OpDefFlags(hnnx::opname_tag_parm_t opstr, int n_in, int n_out)
        : flags(flag_init(opstr, n_in, n_out)),
          opstr_hashval(hnnx::find_opname_hash(opstr))
    {
    }
  ...
  public:
    ...
    {
        return const_cast<OpDefFlags &>(*this).set_flag_state<BIT_in_constmap>(
                val);
    }
    bool set_fake_unsigned(bool val = true)
    {
...
class OpRef final {
  public:
    ...
    int32_t zero_offset(AnyGraphContext c) const
    {
        return output_def(c).zero_offset;
    }
...
class OpDef : public OpDefFlags {
  protected:
    ...
    OpDef(Graph &graph_in, OpId my_id_in, hnnx::opname_tag_parm_t opstr_in,
          OutputDef const &odef, ForConst const &)
        : OpDefFlags(opstr_in, true), splithist(), graphref(graph_in),
          id(my_id_in), opstr(opstr_in), input_defs(), output_def(odef)
    {
    }

  public:
    OpId id;
    const hnnx::opname_tag_t opstr;

    std::vector<OpRef>
            input_defs; // These should be mutable, we mess with them during optimization
    ...
    inline void set_splithist(OpDef const &other)
    {
        splithist = other.splithist;
    }
    // with 0 or 1 output (output_def_in may be null)
    OpDef(Graph &graph_in, OpId my_id_in, hnnx::opname_tag_parm_t opstr_in,
          std::vector<OpRef> &&input_defs_in, OutputDef const *output_def_in,
          hnnx::splithist_t sl)
        : OpDefFlags(opstr_in, input_defs_in.size(),
                     (output_def_in == nullptr) ? 0 : 1),
          splithist(sl), graphref(graph_in), id(my_id_in), opstr(opstr_in),
          input_defs(std::move(input_defs_in)), output_def()
    {
    ...
    OpDef(Graph &graph_in, OpId my_id_in, hnnx::opname_tag_parm_t opstr_in,
        ...
        : OpDefFlags(opstr_in, input_defs_in.size(),
                     (output_def_in == nullptr) ? 0 : 1),
          splithist(), graphref(graph_in), id(my_id_in), opstr(opstr_in),
          input_defs(std::move(input_defs_in)), output_def()
    {
        if (output_def_in != nullptr) {
            output_def = *output_def_in;
    ...
    OpDef make_output_exemplar(OutputDef const *size_from,
                               OutputDef const *outp_from) const;
    // make a copy with the same output and no inputs
    inline OpDef make_output_exemplar() const
    {
    ...
    {
        return output_def.dtype == DType::Multi;
    }
    OpRef reference() const { return OpRef{id, 0}; }
    // these are only safe to use when has_outputs()
    OutputDef &get_outputdef()
    {
        return output_def;
    } //use when need to modify output_def
    OutputDef const &get_outputdef() const
    {
        return output_def;
...
class OpDef_ConstBase : public OpDef {
  ...
  protected:
    OpDef_ConstBase(Graph &graph_in, OpId my_id_in, opname_tag_parm_t opstr,
  ...
  public:
    ...
    inline OpDef_ConstBase(Graph &graph_in, Deserializer &dctx)
  ...
  protected:
    uint32_t
    find_basic_hash() const noexcept; // find the hash of opstr and OutputDef.
...
inline bool compare_constbase_eq(const OpDef_ConstBase &lhs,
                                 const OpDef_ConstBase &rhs)
{
    return lhs.get_content_hash() == rhs.get_content_hash() &&
           compare_constbase(lhs, rhs) == 0;
}

class OpDef_Const : public OpDef_ConstBase {
  public:
    std::unique_ptr<Tensor> const_data;
    OpDef_Const(Graph &graph_in, OpId my_id_in, OutputDef const &output_def,
                const uint8_t *data_in, size_t len);
    OpDef_Const(Graph &graph_in, OpId my_id_in,
                std::unique_ptr<Tensor> tensor_in);
    virtual ~OpDef_Const();

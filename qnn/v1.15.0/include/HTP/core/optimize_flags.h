...
struct OptimFlags {
    ...
    enum f : flags_t {
        ...
        cse_after_always = flagbit<
                1>::val, // always do CSE after a pass containing this rule
        cse_after_if = flagbit<
                2>::val, // always do CSE after the pass, if the rule succeeds.
        cse_set_triggerA =
                flagbit<3>::val, // trigger CSE when this rule succeeds.
        cse_before_if_triggerA = flagbit<
                4>::val, // if triggered, CSE before pass containing this rule
        cse_set_triggerB =
                flagbit<5>::val, // trigger CSE when this rule succeeds.
        cse_before_if_triggerB = flagbit<
                6>::val, // if triggered, CSE before pass containing this rule
        fold_relu_flag = flagbit<7>::
                val, // if overall flag for folding relu is enabled all rules containing this flag will be triggered
        cp_after_always = flagbit<
                8>::val, // always do CP after a pass containing this rule
    ...
    static constexpr flags_t combine_over_pass =
            cse_after_always | cse_before_if_triggerA | cse_before_if_triggerA |
            cp_after_always;
    ...
    class OptFlagState {
      ...
      public:
        ...
        inline void
        update(flags_t previous_pass_flags, // GraphOptPass.flags from the previous pass
        ...
        {
            flags_t trigs = m_trigger;
            flags_t next_trigs =
                    trigs | (success_flags &
                             (any_rule | cse_set_triggerA | cse_set_triggerB));
            bool do_cse = ((next_trigs & any_rule) != 0 &&
                           ((success_flags & cse_after_if) != 0 ||
                            (previous_pass_flags & cse_after_always) != 0 ||
            ...
            bool do_cp = (next_trigs & any_rule) != 0 &&
                         (previous_pass_flags & cp_after_always) != 0;
            if (do_cse) {
    ...
    template <typename U> static constexpr flags_t flag_evaluate()
    {

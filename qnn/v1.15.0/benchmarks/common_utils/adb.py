...
def check_adb_version(adb_path):
    ...
    if returncode is not 0:
        logger.error(
            '%s failed with stderr of: %s' %
            (adb_cmd, cmd_out + cmd_err))
        raise Exception(
            "Failed to perform adb version check using command : %s" %
            adb_cmd)
    adb_version = cmd_out.split('\n')[0].split(' ')[-1]
    if adb_version > recommended_adb_version:
        logger.warning(
            'The version of adb (%s) found at %s has not been validated. Recommended to use known stable adb version %s' %
            (adb_version, adb_path, recommended_adb_version))
    elif not adb_version == recommended_adb_version:
        raise ValueError(
            "sdk-tests require adb version %s. Found adb version %s at %s" %
            (recommended_adb_version, adb_version, adb_path))


class Adb(object):
    def __init__(self, adb_executable, device,
                 master_id=None, hostname='localhost'):
    ...
    def push(self, src, dst, cwd='.'):
        ...
        if (self._execute('shell', ['[ -d %s ]' % dst], cwd=cwd)[0] == 0):
            ...
            if (os.path.basename(src) == os.path.basename(
                    dst)) and not os.path.isdir(src):
        ...
        if code == 0:
            ...
            separator = '/' if 'linux' in sys.platform else '\\'
            if src[-1] == separator:
                src = src[:-1]
            file_name = src.split(separator)[-1]
            # check if destination directory exists
            # if it exists, then append file name to dst
            # otherwise, adb will rename src dir to dst
            if dst_dir_exists:
                dst = (
                    dst + file_name) if dst[-1] == '/' else (dst + '/' + file_name)
            code, out, error = self._execute(
                'shell', [
                    '[ -e %s ]' %
                    dst], cwd=cwd)
        return code, out, err
    ...
    def install(self, apk_path, package_name):
        ...
        if code == 0:
            # We confirm the installation as we can't rely on the result code
            # above
    ...
    def uninstall(self, package_name):
        ...
        if code == 0:
            # We need to validate the output as adb install will return always
            # 0
            code = 0 if out[-1] == 'Success' else -1
        return code, out, err

    def reboot(self, timeout=120):
        command = [self.__adb_executable, '-s', self._adb_device, '-H', self._hostname, 'reboot']
        try:
            out = subprocess.check_output(command, timeout=timeout)
        except (subprocess.TimeoutExpired, subprocess.CalledProcessError) as e:
            logger.error("Device - {} Reboot Failed!!: {}".format(self._adb_device, e.output))
            return False
        return True

    def wait_for_device(self, timeout=120):
        command = [self.__adb_executable, '-s', self._adb_device, '-H', self._hostname, 'wait-for-device']
        try:
            out = subprocess.check_output(command, timeout=timeout)
        except (subprocess.TimeoutExpired, subprocess.CalledProcessError) as e:
            logger.error("Device - {} not avaialble after timeout: {}!!".format(self._adb_device, e.output))
            return False
        return True
    ...
    def _execute(self, command, args, cwd='.', recovery_enable=True):
        adb_command_args = [
            ...
            self._adb_device,
            command] + args
        (return_code, output, error) = execute(self.__adb_executable,
                                               adb_command_args, cwd=cwd, timeout=Timeouts.ADB_DEFAULT_TIMEOUT)

        # when the process gets killed, it will return -9 code; Logging this
        ...
        if self._adb_device:
            ...
            if recovery_enable and issue != "":
                ...
                if device_state:
                    ...
                    logger.info(
                        "Retrying the command after device recovery: {0} {1}".format(
                            self.__adb_executable, adb_command_args))
                    (return_code, output, error) = execute(self.__adb_executable,
                                                           adb_command_args, cwd=cwd, timeout=Timeouts.ADB_DEFAULT_TIMEOUT)
                else:
    ...
    def getLogcat(self, command):
        ...
        exec_command = " ".join(adb_command_args)

        if command == 'logcat -c':
            return subprocess.call(shlex.split(exec_command))
        elif command == 'logcat -d':
            log = subprocess.check_output(shlex.split(exec_command))
            return log
    ...
    def get_devices(self):
        ...
        for line in out:
            ...
            match_obj = re.match(
                r"^([a-zA-Z0-9.]+(:[0-9]+)?)\s+device", line, re.M)
            if match_obj:
    ...
    def get_device_info(self, fatal=True):
        ...
        if ret != 0:
            if fatal is not True:
                logger.warning(
                    'Non fatal get prop call failure, is the target os not Android?')
                return ret, [], err
        if out:
            for line in out:
                line = line.strip()
        ...
        try:
            from .device_utils import get_socid
            soc_id = get_socid(self, logger)
        except Exception as e:
            soc_id = None

        if soc_id is not None:
            dev_info.append(["soc_id", soc_id[0]])
    ...
    def check_file_exists(self, file_path):
        ...
        if 'No such file or directory' in ''.join(
                err) or 'No such file or directory' in ''.join(out):
            ret = 1
        return ret == 0
    ...
    def _is_device_online(self):
        ...
        for line in out:
            match_obj = re.match(
                r"^{0}\s+device".format(self._adb_device), line, re.M)
            if match_obj:
    ...
    def recover_device(self, issue="crash"):
        if not recovery:
            logger.error(
                "DeviceConnector Module not present. Recovery of device not possible")
            return 0

        logger.warning(
            "Trying to recover the device {0} from {1} state".format(
                self._adb_device, issue))

        if not self.__master_id:
            logger.warning(
                "Device master_id is not provided. Retrieving from json file.")
            if os.path.exists(os.path.join(
                    '/local/mnt/workspace', 'recovery_ports.json')):
                with open(os.path.join('/local/mnt/workspace', 'recovery_ports.json')) as master_data_file:
                    ...
                    if self._adb_device in master_data:
                        ...
                        logger.info(
                            "Using Recovery port: {0}".format(
                                self.__master_id))
                    else:
                        raise Exception(
                            "Device master_id is not provided in the json file. Cannot recover the device.")
            else:
                raise Exception(
                    "Mapping JSON file not available. Device cannot be recovered.")

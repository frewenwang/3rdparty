...
class Config(object):
    def __init__(self, cfg_file, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype,
        ...
        if deviceostype and deviceostype not in CONFIG_VALID_DEVICEOSTYPES:
            ...
            self._architectures = [self._product.ARCH_ARM]
            self._platform_os = self._product.PLATFORM_OS_ANDROID
            self._compiler = self._product.COMPILER_CLANG90
            self._stl_library = self._product.STL_LIBCXX_SHARED
            self._artifacts_config = load_json(
                os.path.join(
                    config_prefix,
                    product.PRODUCT_BM,
                    self._product.BENCH_ANDROID_ARM32_ARTIFACTS_JSON))
    ...
    def __override_cfgfile__(self, outputbasedir, devicelist, hostname, iterations):
        ...
        if devicelist is not None:
            ...
            logger.info('Overriding device id to %s, instead of %s from config file' % (
        ...
        if hostname is not None:
            ...
            logger.info('Overriding host name to %s' %
        ...
        if iterations is not None:
            ...
            logger.info('Overriding number of runs to %s' %
    ...
    def __default_artifact_path(self, compiler, artifact, product):
        ...
        if "hexagon" in compiler:
            if self._use_signed_skel:
                return os.path.join(_sdk_root, 'target', compiler, 'lib', 'signed', artifact)
            else:
                return os.path.join(_sdk_root, 'target', compiler, 'lib', 'unsigned', artifact)
        elif _base_name.endswith(".so") or _base_name.startswith("lib"):
            return os.path.join(_sdk_root, 'target', compiler, 'lib', artifact)
        else:
            return os.path.join(_sdk_root, 'target', compiler, 'bin', artifact)
    ...
    def host_artifacts(self):
        ...
        for _compiler, _artifacts in self._artifacts_config[self._product.CONFIG_ARTIFACTS_KEY].items(
            ...
            if _compiler == self._product.CONFIG_ARTIFACTS_COMPILER_KEY_HOST:
                for _artifact_path in _artifacts:
                    if not os.path.isabs(_artifact_path):
                    ...
                    if os.path.exists(_artifact_path):
                        ...
                        if _base_name == self._product.DIAGVIEW_EXE:
    ...
    def artifacts(self):
        arch_map = {
            ...
            'aarch64-qnx': 'dummy',
            'aarch64-linux-oe-gcc9.3': 'dummy',
            'aarch64-linux-oe-gcc8.2': 'dummy'
        }
        _tmp = {}
            ...
            for _artifact_path in _artifacts:
                ...
                if os.path.exists(_artifact_path):
                    ...
                    if _base_name.endswith(
                            ".so") or _base_name.startswith("lib"):
                        ...
                        with open(self.backend_config, 'r') as conf:
                            ...
                            if "backend_extensions" in data_conf.keys():
                                if data_conf['backend_extensions']['config_file_path']:
                                    ...
                                    data_conf['backend_extensions']['config_file_path'] = '/'.join([_dev_lib_dir,os.path.basename(data_conf['backend_extensions']['config_file_path'])])
...
class ConfigFactory(object):
    ...
    @staticmethod
    def make_config(cfgfile, outputbasedir, devicelist, hostname, usedevicesconnected,
        ...
        if usedevicesconnected:
            ret, devicelist, err = Adb(
                'adb', '', hostname=hostname).get_devices()
            if ret != 0:
        ...
        if not os.path.exists(model_path):
            ...
            return Config(cfgfile, _config_from_json, outputbasedir, devicelist, hostname, deviceostype,
                          ...
                          profilinglevel, product, dsp_type, htp_serialized, arm_prepare, use_signed_skel,cache, use_shared_buffer)

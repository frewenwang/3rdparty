...
def bench(program_name, args_list, device_msm_os_dict=None):
    try:
        ...
        benchmarks, results_dir = bm_bm.BenchmarkFactory.make_benchmarks(
            config, product_type_obj, args_parser.device_os_type_override, args_parser.htp_serialized, args_parser.shared_buffer)
        ...
        for device_id in config.devices:
            ...
            _, device_info, _ = device.adb.get_device_info(
                fatal=((args_parser.device_os_type_override != 'le' and args_parser.device_os_type_override != 'le64'\
                        and args_parser.device_os_type_override != 'qnx-aarch64')))

            logger.debug("Perform md5 checksum on %s" % device_id)
            bm_md5.perform_md5_check(device, [item for sublist in config.artifacts.values() for item in sublist]

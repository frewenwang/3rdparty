...
JSONCPP_STRING valueToString(double value,
  ...
  while (true) {
    int len = snprintf(
        ...
        precision, value);
//    (precisionType == PrecisionType::significantDigits) ? "%.*g" : "%.*f", // by liruiyang
    assert(len >= 0);
...
//  if (PrecisionType::precisionType == decimalPlaces) { // by liruiyang
...
//  PrecisionType precisionType(PrecisionType::significantDigits); // by liruiyang
  PrecisionType precisionType(significantDigits);
  if (pt_str == "significant") {
    precisionType = significantDigits;
  } else if (pt_str == "decimal") {
//    precisionType = PrecisionType::decimalPlaces; // by liruiyang
